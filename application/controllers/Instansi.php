...
public function GetListKabKota() {
    $kode_provinsi = $this->input->post('Kode', TRUE) ?: $this->input->get('Kode', TRUE);
    ...
    $prov2 = substr(trim($kode_provinsi), 0, 2);
    $kode_pattern = $prov2 . '.%';
    
    $kabkota = $this->db
        ->select('Kode, Nama')
        ->from('kodewilayah')
        ->where('Kode LIKE', $kode_pattern)
    ...
    public function GetListInstansiLevel4() {
        $kode_wilayah = $this->input->post('kode_wilayah', TRUE) ?: $this->input->get('kode_wilayah', TRUE);
        if (!$kode_wilayah) {
        ...
        $provKode = substr($kode_wilayah, 0, 2);
        $instansi = $this->db->select('id, nama')
            ->from('akun_instansi')
            ->where("(kodewilayah = " . $this->db->escape($kode_wilayah) . " OR kodewilayah = " . $this->db->escape($provKode) . ")")
            ->where('deleted_at IS NULL')
            ->order_by('nama', 'ASC')
            ->get()
            ->result_array();
...
public function PerjanjianKinerjaPD() {
    ...
    $data['KodeWilayah'] = $KodeWilayah;
    $data['InstansiId'] = $instansi_id;
    $data['ActiveInstansiId'] = $filter_instansi_id ?: $instansi_id;
    $data['TahunAktif'] = $tahun;
    $data['TahunList'] = [2025, 2026, 2027, 2028, 2029, 2030];
    $data['IsLoggedIn'] = $is_logged_in;
    $data['IsRole4'] = $is_role_4;
...
public function getSasaranByLevel() {
    ...
    $allowed = ['program', 'kegiatan', 'sub_kegiatan', 'subkegiatan'];
    if (!in_array($level, $allowed)) {
        echo json_encode(['status' => 'error', 'message' => 'Level tidak valid']);
        return;
    }
    if ($level === 'subkegiatan') $level = 'sub_kegiatan';
    
    // Tentukan field target & anggaran berdasarkan tahun
    $target_field = 'target_' . $tahun;
    $anggaran_field = 'anggaran_' . $tahun;
    ...
    try {
        if ($level == 'program') {
            // Ambil semua program
            $this->db->select("p.id, p.kode_program as kode, p.nama as nama, p.outcome as outcome_fallback, p.indikator as ind_fallback, p.satuan as satuan_fallback, p.$target_field as target_fallback, p.$anggaran_field as anggaran_val, COALESCE(NULLIF(k.bidang_sub_koordinator, ''), NULLIF(k.satuan_unit_kerja, ''), k.jabatan) as sub_unit", false);
            $this->db->from('renstra_program p');
            ...
            foreach ($programs as &$prog) {
                ...
                if (!empty($outcomes)) {
                    foreach ($outcomes as &$out) {
                        ...
                        $firstInd = !empty($out_indikators) ? $out_indikators[0] : null;
                        $sasaranProgramList[] = [
                            'id' => $out['id'],
                            'text' => $out['outcome_text'],
                            'indikator' => $firstInd ? $firstInd['indikator'] : '',
                            'satuan' => $firstInd ? $firstInd['satuan'] : '%',
                            'target' => $firstInd ? $firstInd['target'] : ''
                        ];
                        foreach ($out_indikators as $oi) {
                            $all_indikators[] = $oi;
                        }
                    }
                } elseif (!empty($prog['outcome_fallback'])) {
                    $sasaranProgramList[] = [
                        'id' => 'p_' . $prog['id'],
                        'text' => $prog['outcome_fallback'],
                        'indikator' => $prog['ind_fallback'] ?: '',
                        'satuan' => $prog['satuan_fallback'] ?: '%',
                        'target' => $prog['target_fallback'] ?: ''
                    ];
                } else {
                    $sasaranProgramList[] = [
                        'id' => 'p_' . $prog['id'],
                        'text' => 'Meningkatnya kinerja pelaksanaan ' . $prog['nama'],
                        'indikator' => $prog['ind_fallback'] ?: 'Persentase Capaian Target Program',
                        'satuan' => $prog['satuan_fallback'] ?: '%',
                        'target' => $prog['target_fallback'] ?: '100'
                    ];
                }
                
                $prog['outcomes'] = $outcomes;
                $prog['sasaranProgram'] = $sasaranProgramList;
                $prog['sasaran'] = !empty($sasaranProgramList) ? implode('; ', array_column($sasaranProgramList, 'text')) : '';
                $prog['indikator_list'] = $all_indikators;
                $prog['anggaran'] = !empty($prog['anggaran_val']) ? number_format($prog['anggaran_val'], 0, ',', '.') : '';
            }
            $result = $programs;

        } elseif ($level == 'kegiatan') {
            $this->db->select("k.id, k.program_id, k.kode_nomenklatur as kode, k.nama as nama, k.sasaran as sasaran_fallback, k.indikator as ind_fallback, k.satuan as satuan_fallback, k.$target_field as target_fallback, k.$anggaran_field as anggaran_val, p.kode_program as program_kode, p.nama as program_nama, COALESCE(NULLIF(ak.bidang_sub_koordinator, ''), NULLIF(ak.satuan_unit_kerja, ''), ak.jabatan) as sub_unit", false);
            ...
            foreach ($rows as &$row) {
                ...
                if (!empty($sasarans)) {
                    foreach ($sasarans as &$sas) {
                        ...
                        $firstInd = !empty($sas_indikators) ? $sas_indikators[0] : null;
                        $sasaranList[] = [
                            'id' => $sas['id'],
                            'text' => $sas['sasaran_text'],
                            'indikator' => $firstInd ? $firstInd['indikator'] : '',
                            'satuan' => $firstInd ? $firstInd['satuan'] : '%',
                            'target' => $firstInd ? $firstInd['target'] : ''
                        ];
                ...
                } elseif (!empty($row['sasaran_fallback'])) {
                    $sasaranList[] = [
                        'id' => 'k_' . $row['id'],
                        'text' => $row['sasaran_fallback'],
                        'indikator' => $row['ind_fallback'] ?: '',
                        'satuan' => $row['satuan_fallback'] ?: '%',
                        'target' => $row['target_fallback'] ?: ''
                    ];
                } else {
                    $sasaranList[] = [
                        'id' => 'k_' . $row['id'],
                        'text' => 'Terlaksananya ' . $row['nama'],
                        'indikator' => $row['ind_fallback'] ?: 'Persentase Ketercapaian Kegiatan',
                        'satuan' => $row['satuan_fallback'] ?: '%',
                        'target' => $row['target_fallback'] ?: '100'
                    ];
                }
                
                $row['sasaran_list'] = $sasarans;
                $row['sasaranItems'] = $sasaranList;
                $row['sasaran'] = !empty($sasaranList) ? implode('; ', array_column($sasaranList, 'text')) : '';
                $row['indikator_list'] = $all_indikators;
                $row['anggaran'] = !empty($row['anggaran_val']) ? number_format($row['anggaran_val'], 0, ',', '.') : '';
            }
            $result = $rows;

        } elseif ($level == 'sub_kegiatan') {
            $this->db->select("sk.id, sk.kegiatan_id, sk.kode_nomenklatur as kode, sk.nama as nama, sk.sasaran as sasaran_fallback, sk.indikator as ind_fallback, sk.satuan as satuan_fallback, sk.$target_field as target_fallback, sk.$anggaran_field as anggaran_val, k.nama as kegiatan_nama, k.kode_nomenklatur as kegiatan_kode, p.id as program_id, p.kode_program as program_kode, p.nama as program_nama, COALESCE(NULLIF(ak.bidang_sub_koordinator, ''), NULLIF(ak.satuan_unit_kerja, ''), ak.jabatan) as sub_unit", false);
            ...
            foreach ($rows as &$row) {
                ...
                if (!empty($sasarans)) {
                    ...
                    $sasaranList[] = [
                        'id' => 'sk_' . $row['id'],
                        'text' => 'Tersedianya ' . $row['nama'],
                        'indikator' => $row['ind_fallback'] ?: 'Jumlah Dokumen / Laporan / Output',
                        'satuan' => $row['satuan_fallback'] ?: 'Dokumen',
                        'target' => $row['target_fallback'] ?: '1'
                    ];
                }
...
/**
 * Simpan Perjanjian Kinerja (AJAX) - Tanpa Perlu Upload File
 */
public function simpanPerjanjianKinerja() {
    ...
    $id = (int)$this->input->post('id', TRUE);
    $mode = $this->input->post('mode', TRUE) ?: 'new';
    ...
    $jenis_perjanjian = strtoupper(trim($this->input->post('jenis_perjanjian', TRUE)));
    ...
    $sasaran_data = $this->input->post('sasaran_data', FALSE); // Raw JSON string
    $sub_unit = trim($this->input->post('sub_unit', TRUE));
    
    // Normalisasi jenis perjanjian
    if ($jenis_perjanjian === 'PK MURNI' || $jenis_perjanjian === 'DEFINITIF') {
        $db_jenis = 'PK Murni';
    } elseif ($jenis_perjanjian === 'PK PERUBAHAN') {
        $db_jenis = 'PK Perubahan';
    } elseif ($jenis_perjanjian === 'PK PLT') {
        $db_jenis = 'PK PLT';
    } else {
        $db_jenis = 'PK Murni';
    }
    
    if (!$pegawai_pengampu_id || !$jenis_perjanjian || !$periode_awal || !$periode_akhir || !$sasaran_level || empty($sasaran_data)) {
        echo json_encode(['status' => 'error', 'message' => 'Semua ketentuan dan sasaran perjanjian wajib diisi!']);
        return;
    }
    ...
    // Jika mode adalah addpk / edit pada data yang sudah ada
    if ($id > 0) {
        ...
        if (!$existing || $existing->id_instansi != $instansi_id || $existing->kode_wilayah != $KodeWilayah) {
            echo json_encode(['status' => 'error', 'message' => 'Akses ditolak! Data tidak ditemukan.']);
            return;
        }
        ...
        if ($db_jenis === 'PK Perubahan') {
            $updateData['pk_perubahan_doc_id'] = $existing->pk_perubahan_doc_id ?: $doc_id_str;
            $updateData['pk_perubahan_data'] = $sasaran_data;
            $updateData['status_perubahan'] = 'menunggu';
        } elseif ($db_jenis === 'PK PLT') {
            $updateData['pk_plt_doc_id'] = $existing->pk_plt_doc_id ?: $doc_id_str;
            $updateData['pk_plt_data'] = $sasaran_data;
            ...
            $updateData['sasaran_level'] = $sasaran_level;
            $updateData['sub_unit'] = $sub_unit;
            $updateData['status'] = 'menunggu';
        }
        
        $this->db->where('id', $id)->update('perjanjian_kinerja', $updateData);
        echo json_encode(['status' => 'success', 'message' => $db_jenis . ' berhasil disimpan dengan status Menunggu Verifikasi']);
        return;
    }
    ...
    $insertData = [
        ...
        'jenis_perjanjian' => $db_jenis,
        'periode_awal' => $periode_awal,
        'periode_akhir' => $periode_akhir,
        'anggaran' => $anggaran,
        'sasaran_level' => $sasaran_level,
        'sasaran_data' => ($db_jenis === 'PK Murni') ? $sasaran_data : null,
        'pk_perubahan_data' => ($db_jenis === 'PK Perubahan') ? $sasaran_data : null,
        'pk_plt_data' => ($db_jenis === 'PK PLT') ? $sasaran_data : null,
        'sub_unit' => $sub_unit,
        'status' => 'menunggu',
        'status_perubahan' => ($db_jenis === 'PK Perubahan') ? 'menunggu' : null,
        'status_plt' => ($db_jenis === 'PK PLT') ? 'menunggu' : null,
        'definitif_doc_id' => ($db_jenis === 'PK Murni') ? $doc_id_str : null,
        'pk_perubahan_doc_id' => ($db_jenis === 'PK Perubahan') ? $doc_id_str : null,
        'pk_plt_doc_id' => ($db_jenis === 'PK PLT') ? $doc_id_str : null,
        'created_at' => date('Y-m-d H:i:s'),
        'updated_at' => date('Y-m-d H:i:s')
    ];
    
    $this->db->insert('perjanjian_kinerja', $insertData);
    if ($this->db->affected_rows() > 0) {
        echo json_encode(['status' => 'success', 'message' => 'Draft Perjanjian Kinerja berhasil disimpan dengan status Menunggu']);
    } else {
...
/**
 * Get Perjanjian Kinerja by ID (AJAX) - untuk edit & Document Viewer
 */
public function getPerjanjianKinerja() {
    ...
    if ($data) {
        // Ambil nama wilayah
        $wilayah = $this->db->select('Nama')->where('Kode', $data['kode_wilayah'])->get('kodewilayah')->row_array();
        $data['nama_wilayah'] = $wilayah ? $wilayah['Nama'] : 'Pemerintah Daerah';
...
public function updatePerjanjianKinerja() {
    ...
    $anggaran = $this->input->post('anggaran', TRUE) ? 1 : 0;
    $sasaran_level = $this->input->post('sasaran_level', TRUE);
    $sasaran_data = $this->input->post('sasaran_data', FALSE);
    $sub_unit = trim($this->input->post('sub_unit', TRUE));
    
    $updateData = [
        'pegawai_pengampu_id' => $pegawai_pengampu_id ?: $existing->pegawai_pengampu_id,
        'atasan_langsung_id' => $atasan_langsung_id ?: $existing->atasan_langsung_id,
        'periode_awal' => $periode_awal ?: $existing->periode_awal,
        'periode_akhir' => $periode_akhir ?: $existing->periode_akhir,
        'anggaran' => $anggaran,
        'sasaran_level' => $sasaran_level ?: $existing->sasaran_level,
        'sub_unit' => $sub_unit ?: $existing->sub_unit,
        'updated_at' => date('Y-m-d H:i:s')
    ];
    
    if ($jenis_perjanjian === 'PK PERUBAHAN') {
        $updateData['pk_perubahan_data'] = $sasaran_data;
    } elseif ($jenis_perjanjian === 'PK PLT') {
        $updateData['pk_plt_data'] = $sasaran_data;
    } else {
...
/**
 * Hapus Dokumen PK (Definitif / Perubahan / PLT) atau Hapus Row
 */
public function hapusDokumenPerjanjianKinerja() {
    if (!$this->input->is_ajax_request()) show_404();
    if (!$this->can_crud() || !$this->is_role_4()) {
        echo json_encode(['status' => 'error', 'message' => 'Akses ditolak!']);
        return;
    ...
    $id = (int)$this->input->post('id', TRUE);
    $doctype = strtolower(trim($this->input->post('doctype', TRUE)));
    ...
    if ($doctype === 'perubahan') {
        $this->db->where('id', $id)->update('perjanjian_kinerja', [
            ...
            'pk_perubahan_data' => null,
            'status_perubahan' => null,
            'updated_at' => date('Y-m-d H:i:s')
        ]);
        echo json_encode(['status' => 'success', 'message' => 'Dokumen PK Perubahan berhasil dihapus']);
    } elseif ($doctype === 'plt') {
        $this->db->where('id', $id)->update('perjanjian_kinerja', [
            'pk_plt_doc_id' => null,
            'pk_plt_data' => null,
        ...
        echo json_encode(['status' => 'success', 'message' => 'Dokumen PK PLT berhasil dihapus']);
    } elseif ($doctype === 'definitif') {
        $this->db->where('id', $id)->update('perjanjian_kinerja', [
        ...
        echo json_encode(['status' => 'success', 'message' => 'Dokumen Definitif berhasil dihapus']);
    } else {
        $this->db->where('id', $id)->update('perjanjian_kinerja', ['deleted_at' => date('Y-m-d H:i:s')]);
        echo json_encode(['status' => 'success', 'message' => 'Perjanjian Kinerja berhasil dihapus']);
    }
}
...
public function updateStatusPerjanjianKinerja() {
    ...
    $doctype = strtolower(trim($this->input->post('doctype', TRUE))) ?: 'definitif';
    ...
    if ($doctype === 'perubahan') {
        $updateData['status_perubahan'] = $status;
        if ($status === 'disetujui' && empty($existing->pk_perubahan_doc_id)) {
            $updateData['pk_perubahan_doc_id'] = (string)rand(2060, 2999);
        }
    } elseif ($doctype === 'plt') {
        $updateData['status_plt'] = $status;
        if ($status === 'disetujui' && empty($existing->pk_plt_doc_id)) {
            $updateData['pk_plt_doc_id'] = (string)rand(2060, 2999);
        }
    } else {
        $updateData['status'] = $status;
        if ($status === 'disetujui' && empty($existing->definitif_doc_id)) {
            $updateData['definitif_doc_id'] = (string)rand(2060, 2999);
        }
    }
    ...
    public function BelanjaSubKegiatan() {
        ...
        // Tentukan active instansi ID
        $active_instansi_id = null;
        if ($is_role_4 && !empty($instansi_id)) {
            $active_instansi_id = (int)$instansi_id;
        } elseif (!empty($filter_instansi)) {
            $active_instansi_id = (int)$filter_instansi;
        } elseif (!empty($instansi_id)) {
            $active_instansi_id = (int)$instansi_id;
        } elseif ($is_logged_in && !empty($Data['ListInstansi'])) {
            $active_instansi_id = (int)$Data['ListInstansi'][0]['id'];
        } elseif (!$is_logged_in && !empty($KodeWilayah) && !empty($Data['ListInstansi'])) {
            $active_instansi_id = (int)$Data['ListInstansi'][0]['id'];
        }

        $currentInstansi = null;
        if (!empty($active_instansi_id)) {
            $currentInstansi = $this->db->where('id', $active_instansi_id)->where('deleted_at IS NULL')->get('akun_instansi')->row_array();
        }
        $Data['CurrentInstansi'] = $currentInstansi;
        $Data['ActiveInstansiId'] = $active_instansi_id;
        
        // Ambil Sub Unit untuk Instansi ini
        $subUnits = [];
        if (!empty($active_instansi_id)) {
            $subUnits = $this->db->select('id, kode_sub_unit, nama_sub_unit, bidang_urusan_id, instansi_id')
                ->from('sub_unit')
                ->where('instansi_id', $active_instansi_id)
                ->where('deleted_at IS NULL')
                ->order_by('urutan', 'ASC')
                ->order_by('kode_sub_unit', 'ASC')
                ->get()
                ->result_array();
        }
        if (empty($subUnits) && !empty($currentInstansi)) {
            $subUnits[] = [
                'id' => 0,
                'kode_sub_unit' => !empty($currentInstansi['kode_instansi']) ? $currentInstansi['kode_instansi'] . '.0001' : '01.0001',
                'nama_sub_unit' => $currentInstansi['nama'] ?: 'SEKRETARIAT',
                'bidang_urusan_id' => '',
                'instansi_id' => $currentInstansi['id']
            ];
        }
        ...
        // Ambil Data Belanja
        if (!$is_logged_in && empty($KodeWilayah)) {
            $belanjaData = [];
        } else {
            ...
            if (!empty($active_instansi_id)) {
                $this->db->where('h.id_instansi', (int)$active_instansi_id);
            }
    ...
    public function GetBelanjaData() {
        ...
        $filter_instansi = $this->input->post('instansi', TRUE) ?: $this->input->post('instansi_id', TRUE);
        
        if ($is_role_4 && $instansi_id) {
            $filter_instansi = $instansi_id;
        }

        if (!$this->is_logged_in() && empty($KodeWilayah)) {
    ...
    // ================================================================
    // GET SUB UNIT BY INSTANSI (AJAX)
    // ================================================================
    public function GetSubUnitsByInstansi() {
        if (!$this->input->is_ajax_request()) {
            show_404();
            return;
        }
        ...
        $is_role_4 = $this->is_role_4();
        if ($is_role_4 && $this->get_instansi_id()) {
            $instansi_id = $this->get_instansi_id();
        }

        if ($instansi_id <= 0) {
            echo json_encode(['status' => 'error', 'message' => 'Instansi tidak valid', 'data' => []]);
            return;
        }

        $inst = $this->db->where('id', $instansi_id)->where('deleted_at IS NULL')->get('akun_instansi')->row_array();
        if (!$inst) {
            echo json_encode(['status' => 'error', 'message' => 'Instansi tidak ditemukan', 'data' => []]);
            return;
        }
        ...
        if (empty($subUnits)) {
            $subUnits[] = [
                'id' => 0,
                'kode_sub_unit' => !empty($inst['kode_instansi']) ? $inst['kode_instansi'] . '.0001' : '01.0001',
                'nama_sub_unit' => $inst['nama'] ?: 'SEKRETARIAT',
                'bidang_urusan_id' => '',
                'instansi_id' => $inst['id']
            ];
        }

        echo json_encode(['status' => 'success', 'instansi' => $inst, 'data' => $subUnits]);
    }

    // ================================================================
    // GET CASCADE NOMENKLATUR (BIDANG, PROGRAM, KEGIATAN, SUB KEGIATAN DARI RANHIR / RENJA)
    // ================================================================
    // ================================================================
    // GET CASCADE NOMENKLATUR (BIDANG, PROGRAM, KEGIATAN, SUB KEGIATAN DARI RANKHIR RENJA)
    // ================================================================
    public function GetCascadeBidangUrusan() {
        if (!$this->input->is_ajax_request()) {
            show_404();
            return;
        }
        ...
        $sub_unit_id = (int)$this->input->post('sub_unit_id', TRUE);
        $bidang_urusan_csv = trim($this->input->post('bidang_urusan_id', TRUE) ?: '');
        $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: $this->get_instansi_id());
        $is_role_4 = $this->is_role_4();
        if ($is_role_4 && $this->get_instansi_id()) {
            $instansi_id = (int)$this->get_instansi_id();
        }
        $tahun = (int)($this->input->post('tahun', TRUE) ?: date('Y'));
        $KodeWilayah = $this->get_kode_wilayah();
        
        if ($sub_unit_id > 0 && empty($bidang_urusan_csv)) {
            $su = $this->db->select('bidang_urusan_id, instansi_id')->where('id', $sub_unit_id)->get('sub_unit')->row_array();
            if ($su) {
                if (!empty($su['bidang_urusan_id'])) {
                    $bidang_urusan_csv = $su['bidang_urusan_id'];
                }
                if (empty($instansi_id) && !empty($su['instansi_id'])) {
                    $instansi_id = (int)$su['instansi_id'];
                }
            }
        }

        $list = [];
        $filterKodes = !empty($bidang_urusan_csv) ? array_filter(array_map('trim', explode(',', $bidang_urusan_csv))) : [];

        $instRow = null;
        if ($instansi_id > 0) {
            $instRow = $this->db->select('id, nama, kode_instansi, bidang_urusan_id')->where('id', $instansi_id)->get('akun_instansi')->row_array();
            if ($instRow && empty($filterKodes) && !empty($instRow['bidang_urusan_id'])) {
                $filterKodes = array_filter(array_map('trim', explode(',', $instRow['bidang_urusan_id'])));
            }
        }
        ...
        // 1. Ambil dari belanja_sub_kegiatan_header (Prioritas Data Aktual)
        if ($instansi_id > 0) {
            $this->db->select('DISTINCT(kode_bidang_urusan) as kode, nama_bidang_urusan as nama')
                ->from('belanja_sub_kegiatan_header')
                ->where('id_instansi', $instansi_id)
                ->where('deleted_at IS NULL')
                ->where('kode_bidang_urusan IS NOT NULL')
                ->where('kode_bidang_urusan !=', '');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            $bHeaders = $this->db->get()->result_array();
            foreach ($bHeaders as $bh) {
                $bKode = trim($bh['kode']);
                if (!empty($bKode) && !isset($mapBidang[$bKode])) {
                    $bNama = !empty($bh['nama']) ? $bh['nama'] : (isset($nomen_map[$bKode]) ? $nomen_map[$bKode] : 'BIDANG ' . $bKode);
                    $mapBidang[$bKode] = ['kode' => $bKode, 'nama' => $bNama];
                }
            }

            // 2. Ambil dari renstra_program (Pohon Kinerja / Renstra / Rankhir Renja)
            $this->db->select('DISTINCT(rp.kode_program), rp.nama as nama_program')
                ->from('renstra_program rp')
                ->join('renstra_sasaran rs', 'rs.id = rp.sasaran_id')
                ->join('renstra_tujuan rt', 'rt.id = rs.tujuan_id')
            ...
            $renstraProgs = $this->db->get()->result_array();

            foreach ($renstraProgs as $p) {
                $kParts = explode('.', trim($p['kode_program']));
                if (count($kParts) >= 2) {
                    $bKode = $kParts[0] . '.' . $kParts[1];
                    if (!isset($mapBidang[$bKode])) {
                        $bNama = isset($nomen_map[$bKode]) ? $nomen_map[$bKode] : $this->_getBidangName($bKode, $bKode);
                        $mapBidang[$bKode] = ['kode' => $bKode, 'nama' => $bNama];
                    }
                }
            }

            // 3. Ambil dari rancangan_akhir_renja_header / renja_pd_header
            $this->db->select('kode_rekening, sasaran, tujuan')
                ->from('rancangan_akhir_renja_header')
                ->where('deleted_at IS NULL')
                ->where('id_instansi', $instansi_id);
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            ...
            $ranhirRows = $this->db->get()->result_array();
            ...
            foreach ($ranhirRows as $r) {
                $kParts = explode('.', trim($r['kode_rekening']));
                if (count($kParts) >= 2) {
                    $bKode = $kParts[0] . '.' . $kParts[1];
                    if (!isset($mapBidang[$bKode])) {
                        $bNama = !empty($r['sasaran']) ? $r['sasaran'] : (!empty($r['tujuan']) ? $r['tujuan'] : (isset($nomen_map[$bKode]) ? $nomen_map[$bKode] : $this->_getBidangName($bKode, $bKode)));
                        $mapBidang[$bKode] = ['kode' => $bKode, 'nama' => $bNama];
                    }
                }
            }

            // Jika filterKodes ada dan beberapa item di mapBidang cocok, pastikan tidak tereliminasi jika ada di belanja_sub_kegiatan_header
            if (!empty($filterKodes) && !empty($mapBidang)) {
                $filtered = [];
                foreach ($mapBidang as $bk => $bv) {
                    if (in_array($bk, $filterKodes) || isset($bHeaders)) {
                        $filtered[$bk] = $bv;
                    }
                }
                if (!empty($filtered)) {
        ...
        $list = array_values($mapBidang);
        
        // 4. Fallback jika ranhir/renja belum ada data sama sekali
        if (empty($list)) {
            if (!empty($filterKodes)) {
                $this->db->select('Kode as kode, Nomenklatur as nama')
                    ->from('nomenklaturkabupaten')
                    ->where_in('Kode', $filterKodes);
                if ($this->db->field_exists('deleted_at', 'nomenklaturkabupaten')) {
                    $this->db->where('deleted_at IS NULL', null, false);
                }
                $list = $this->db->order_by('Kode', 'ASC')->get()->result_array();
            } else if ($instRow && !empty($instRow['kode_instansi'])) {
                $kParts = explode('.', $instRow['kode_instansi']);
                if (count($kParts) >= 2) {
                    $defBidang = $kParts[0] . '.' . $kParts[1];
                    $this->db->select('Kode as kode, Nomenklatur as nama')
                        ->from('nomenklaturkabupaten')
                        ->where('Kode', $defBidang);
                    if ($this->db->field_exists('deleted_at', 'nomenklaturkabupaten')) {
                        $this->db->where('deleted_at IS NULL', null, false);
                    }
                    $list = $this->db->order_by('Kode', 'ASC')->get()->result_array();
    ...
    public function GetCascadeProgram() {
        if (!$this->input->is_ajax_request()) {
            show_404();
            return;
        }
        header('Content-Type: application/json');
        $bidang_kode = trim($this->input->post('kode_bidang_urusan', TRUE) ?: '');
        $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: $this->get_instansi_id());
        $is_role_4 = $this->is_role_4();
        if ($is_role_4 && $this->get_instansi_id()) {
            $instansi_id = (int)$this->get_instansi_id();
        ...
        // 1. Ambil dari belanja_sub_kegiatan_header
        if ($instansi_id > 0) {
            $this->db->select('DISTINCT(kode_program) as kode, nama_program as nama')
                ->from('belanja_sub_kegiatan_header')
                ->where('id_instansi', $instansi_id)
                ->where('deleted_at IS NULL')
                ->where('kode_program IS NOT NULL')
                ->where('kode_program !=', '')
                ->like('kode_program', $bidang_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            $pHeaders = $this->db->get()->result_array();
            foreach ($pHeaders as $ph) {
                $pKode = trim($ph['kode']);
                if (!empty($pKode) && !isset($mapProg[$pKode])) {
                    $pNama = !empty($ph['nama']) ? $ph['nama'] : (isset($nomen_map[$pKode]) ? $nomen_map[$pKode] : 'PROGRAM ' . $pKode);
                    $mapProg[$pKode] = ['kode' => $pKode, 'nama' => $pNama];
                }
            }

            // 2. Cek dari renstra_program (Rankhir Renja)
            $this->db->select('DISTINCT(rp.kode_program), rp.nama as nama_program')
                ->from('renstra_program rp')
                ->join('renstra_sasaran rs', 'rs.id = rp.sasaran_id')
                ->join('renstra_tujuan rt', 'rt.id = rs.tujuan_id')
                ->where('rt.id_instansi', $instansi_id)
                ->where('rp.deleted_at IS NULL')
                ->like('rp.kode_program', $bidang_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('rt.kode_wilayah', $KodeWilayah);
            }
            $renstraProgs = $this->db->get()->result_array();
            ...
            foreach ($renstraProgs as $p) {
                ...
                if (!isset($mapProg[$pKode])) {
                    $pNama = !empty($p['nama_program']) ? $p['nama_program'] : (isset($nomen_map[$pKode]) ? $nomen_map[$pKode] : 'PROGRAM ' . $pKode);
                    $mapProg[$pKode] = ['kode' => $pKode, 'nama' => $pNama];
                }
            }

            // 3. Cek dari rancangan_akhir_renja_header / renja_pd_header
            $this->db->select('kode_rekening, program')
                ->from('rancangan_akhir_renja_header')
                ->where('deleted_at IS NULL')
                ->where('id_instansi', $instansi_id)
                ->like('kode_rekening', $bidang_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            foreach ($ranhirRows as $r) {
                $parts = explode('.', trim($r['kode_rekening']));
                if (count($parts) >= 3) {
                    $pKode = $parts[0] . '.' . $parts[1] . '.' . $parts[2];
                    if (!isset($mapProg[$pKode])) {
                        $pNama = !empty($r['program']) ? $r['program'] : (isset($nomen_map[$pKode]) ? $nomen_map[$pKode] : 'PROGRAM ' . $pKode);
                        $mapProg[$pKode] = ['kode' => $pKode, 'nama' => $pNama];
                    }
                }
            }
        ...
        $list = array_values($mapProg);

        // 4. Fallback jika ranhir/renja belum ada
        if (empty($list)) {
            $this->db->select('Kode as kode, Nomenklatur as nama')
                ->from('nomenklaturkabupaten')
    ...
    public function GetCascadeKegiatan() {
        if (!$this->input->is_ajax_request()) {
            show_404();
            return;
        }
        header('Content-Type: application/json');
        $program_kode = trim($this->input->post('kode_program', TRUE) ?: '');
        $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: $this->get_instansi_id());
        $is_role_4 = $this->is_role_4();
        if ($is_role_4 && $this->get_instansi_id()) {
            $instansi_id = (int)$this->get_instansi_id();
        ...
        // 1. Ambil dari belanja_sub_kegiatan_header
        if ($instansi_id > 0) {
            $this->db->select('DISTINCT(kode_kegiatan) as kode, nama_kegiatan as nama')
                ->from('belanja_sub_kegiatan_header')
                ->where('id_instansi', $instansi_id)
                ->where('deleted_at IS NULL')
                ->where('kode_kegiatan IS NOT NULL')
                ->where('kode_kegiatan !=', '')
                ->like('kode_kegiatan', $program_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            $kHeaders = $this->db->get()->result_array();
            foreach ($kHeaders as $kh) {
                $kKode = trim($kh['kode']);
                if (!empty($kKode) && !isset($mapKeg[$kKode])) {
                    $kNama = !empty($kh['nama']) ? $kh['nama'] : (isset($nomen_map[$kKode]) ? $nomen_map[$kKode] : 'KEGIATAN ' . $kKode);
                    $mapKeg[$kKode] = ['kode' => $kKode, 'nama' => $kNama];
                }
            }
            ...
            $renstraKegs = $this->db->get()->result_array();

            foreach ($renstraKegs as $k) {
                $kKode = trim($k['kode_nomenklatur']);
                if (!isset($mapKeg[$kKode])) {
                    $kNama = !empty($k['nama_kegiatan']) ? $k['nama_kegiatan'] : (isset($nomen_map[$kKode]) ? $nomen_map[$kKode] : 'KEGIATAN ' . $kKode);
                    $mapKeg[$kKode] = ['kode' => $kKode, 'nama' => $kNama];
                }
            }

            // 3. Cek dari rancangan_akhir_renja_header / renja_pd_header
            $this->db->select('kode_rekening, kegiatan')
                ->from('rancangan_akhir_renja_header')
                ->where('deleted_at IS NULL')
                ->where('id_instansi', $instansi_id)
                ->like('kode_rekening', $program_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            foreach ($ranhirRows as $r) {
                $parts = explode('.', trim($r['kode_rekening']));
                if (count($parts) >= 4) {
                    $kKode = implode('.', array_slice($parts, 0, 4));
                    if (!isset($mapKeg[$kKode])) {
                        $kNama = !empty($r['kegiatan']) ? $r['kegiatan'] : (isset($nomen_map[$kKode]) ? $nomen_map[$kKode] : 'KEGIATAN ' . $kKode);
                        $mapKeg[$kKode] = ['kode' => $kKode, 'nama' => $kNama];
                    }
                }
            }
    ...
    public function GetCascadeSubKegiatan() {
        if (!$this->input->is_ajax_request()) {
            show_404();
            return;
        }
        header('Content-Type: application/json');
        $kegiatan_kode = trim($this->input->post('kode_kegiatan', TRUE) ?: '');
        $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: $this->get_instansi_id());
        $is_role_4 = $this->is_role_4();
        if ($is_role_4 && $this->get_instansi_id()) {
            $instansi_id = (int)$this->get_instansi_id();
        ...
        // 1. Ambil dari belanja_sub_kegiatan_header
        if ($instansi_id > 0) {
            $this->db->select('DISTINCT(kode_sub_kegiatan) as kode, nama_sub_kegiatan as nama')
                ->from('belanja_sub_kegiatan_header')
                ->where('id_instansi', $instansi_id)
                ->where('deleted_at IS NULL')
                ->where('kode_sub_kegiatan IS NOT NULL')
                ->where('kode_sub_kegiatan !=', '')
                ->like('kode_sub_kegiatan', $kegiatan_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            $skHeaders = $this->db->get()->result_array();
            foreach ($skHeaders as $skh) {
                $skKode = trim($skh['kode']);
                if (!empty($skKode) && !isset($mapSubkeg[$skKode])) {
                    $skNama = !empty($skh['nama']) ? $skh['nama'] : (isset($nomen_map[$skKode]) ? $nomen_map[$skKode] : 'SUB KEGIATAN ' . $skKode);
                    $mapSubkeg[$skKode] = ['kode' => $skKode, 'nama' => $skNama];
                }
            }

            // 2. Cek dari renstra_sub_kegiatan (Rankhir Renja)
            $this->db->select('DISTINCT(rsk.kode_nomenklatur), rsk.nama as nama_sub_kegiatan')
                ->from('renstra_sub_kegiatan rsk')
                ->join('renstra_kegiatan rk', 'rk.id = rsk.kegiatan_id')
                ->join('renstra_program rp', 'rp.id = rk.program_id')
                ->join('renstra_sasaran rs', 'rs.id = rp.sasaran_id')
                ...
                ->where('rt.id_instansi', $instansi_id)
                ->where('rsk.deleted_at IS NULL')
                ->like('rsk.kode_nomenklatur', $kegiatan_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('rt.kode_wilayah', $KodeWilayah);
            }
            $renstraSubkegs = $this->db->get()->result_array();

            foreach ($renstraSubkegs as $sk) {
                $skKode = trim($sk['kode_nomenklatur']);
                if (!isset($mapSubkeg[$skKode])) {
                    $skNama = !empty($sk['nama_sub_kegiatan']) ? $sk['nama_sub_kegiatan'] : (isset($nomen_map[$skKode]) ? $nomen_map[$skKode] : 'SUB KEGIATAN ' . $skKode);
                    $mapSubkeg[$skKode] = ['kode' => $skKode, 'nama' => $skNama];
                }
            }

            // 3. Cek dari rancangan_akhir_renja_header / renja_pd_header
            $this->db->select('kode_rekening, sub_kegiatan, kegiatan')
                ->from('rancangan_akhir_renja_header')
                ->where('deleted_at IS NULL')
                ->where('id_instansi', $instansi_id)
                ->like('kode_rekening', $kegiatan_kode . '.', 'after');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            }
            if (!empty($tahun)) {
            ...
            foreach ($ranhirRows as $r) {
                $parts = explode('.', trim($r['kode_rekening']));
                if (count($parts) >= 5 || !empty($r['sub_kegiatan'])) {
                    $skKode = trim($r['kode_rekening']);
                    if (!isset($mapSubkeg[$skKode])) {
                        $skNama = !empty($r['sub_kegiatan']) ? $r['sub_kegiatan'] : (!empty($r['kegiatan']) ? $r['kegiatan'] : (isset($nomen_map[$skKode]) ? $nomen_map[$skKode] : 'SUB KEGIATAN ' . $skKode));
                        $mapSubkeg[$skKode] = ['kode' => $skKode, 'nama' => $skNama];
                    }
                }
            }
    ...
    public function DPA() {
        ...
        if ($is_role_4 && !empty($instansi_id)) {
            $Data['ListInstansi'] = $this->db
                ->select('id, nama')
                ->from('akun_instansi')
                ->where('id', (int)$instansi_id)
                ->where('deleted_at IS NULL')
                ->get()
                ->result_array();
        } elseif (!empty($KodeWilayah)) {
            $provKode = substr($KodeWilayah, 0, 2);
            $Data['ListInstansi'] = $this->db
                ->select('id, nama')
                ->from('akun_instansi')
        ...
        try {
            ...
            $normalizeCode = function($code) {
                ...
                foreach ($parts as $p) {
                    $pTrim = trim($p);
                    if (is_numeric($pTrim)) {
                        $clean[] = (string)(int)$pTrim;
                    } else {
                        $clean[] = strtolower($pTrim);
                    }
                }
                return implode('.', $clean);
            };

            // Ambil semua data RAK yang ada untuk sub kegiatan dan tahun ini
            $this->db->select('*')
                ->from('dpa_rak_rincian')
                ->where('tahun', $tahun)
                ->where('deleted_at IS NULL');
            if (!empty($KodeWilayah)) {
                $this->db->where('kode_wilayah', $KodeWilayah);
            ...
            $normSubKode = $normalizeCode($kode_sub_kegiatan);
            ...
            if (!empty($rincian_list) && is_array($rincian_list)) {
                foreach ($rincian_list as $item) {
                    ...
                    // Cari record yang cocok di database
                    $matchedIds = [];
                    foreach ($existingRows as $er) {
                        $match = false;
                        if ($rincian_id > 0 && !empty($er['rincian_id']) && (int)$er['rincian_id'] === $rincian_id) {
                            $match = true;
                        } else if ($normalizeCode($er['kode_sub_kegiatan']) === $normSubKode && trim($er['kode_rekening']) === $kode_rek) {
                            if (empty($uraian) || empty($er['uraian']) || trim($er['uraian']) === $uraian) {
                                $match = true;
                            }
                        }
                        if ($match) {
                            $matchedIds[] = (int)$er['id'];
                        }
                    }
                    ...
                    if (!empty($matchedIds)) {
                        ...
                        $this->db->where('id', $primaryId)->update('dpa_rak_rincian', $data_rak);

                        // Hapus duplikasi jika ada lebih dari 1 baris
                        if (count($matchedIds) > 1) {
                            $dupIds = array_slice($matchedIds, 1);
                            $this->db->where_in('id', $dupIds)->update('dpa_rak_rincian', ['deleted_at' => date('Y-m-d H:i:s')]);
                        }
                    } else {
    ...
    private function _buildDPAHierarchy($KodeWilayah, $tahun, $filter_instansi = null) {
        $instRow = null;
        if (!empty($filter_instansi)) {
            $instRow = $this->db->select('id, nama, kode_instansi')->where('id', (int)$filter_instansi)->get('akun_instansi')->row_array();
        }

        // Ambil nama wilayah jika ada
        $namaWilayah = 'Kab. Cirebon';
        if (!empty($KodeWilayah)) {
            $wRow = $this->db->select('Nama')->where('Kode', $KodeWilayah)->get('kodewilayah')->row_array();
            if ($wRow && !empty($wRow['Nama'])) {
                $namaWilayah = $wRow['Nama'];
            }
        }

        // 1. Ambil data dari belanja_sub_kegiatan_header
        $this->db->select('*')
            ->from('belanja_sub_kegiatan_header')
            ->where('deleted_at IS NULL');
        ...
        if (!empty($filter_instansi)) {
            ...
            if ($instRow && !empty($instRow['nama'])) {
                $this->db->or_where('nama_perangkat_daerah', $instRow['nama']);
            }
            $this->db->group_end();
        }
        ...
        // 2. Ambil data dari renja_pd_header & renja_pd_detail
        $this->db->select('h.*, d.dpa_murni_rp, d.dpa_perubahan_rp, d.renja_rp, d.sumber_dana, d.lokasi, d.indikator_kinerja, d.satuan')
            ->from('renja_pd_header h')
            ->join('renja_pd_detail d', 'd.header_id = h.id AND d.deleted_at IS NULL', 'left')
            ->where('h.deleted_at IS NULL');
        ...
        $renjaRows = $this->db->order_by('h.kode_rekening', 'ASC')->get()->result_array();
        ...
        $rakSaved = $this->db->get()->result_array();
        ...
        foreach ($rakSaved as $rs) {
            ...
            if (!isset($rakMapBySubKegKode[$rs['kode_sub_kegiatan']])) {
                $rakMapBySubKegKode[$rs['kode_sub_kegiatan']] = $rs;
            }
        }

        // Ambil pemetaan Lokasi & Sumber Dana dari Rankhir Renja (rancangan_akhir_renja_sub_kegiatan_data)
        $rankhirLokasiMap = [];
        $rankhirSumberDanaMap = [];

        $this->db->select('rsk.kode_nomenklatur, rsub.kab_kota, rsub.kecamatan, rsub.desa, rsub.lokasi_pelaksanaan, rsub.sumber_dana')
            ->from('rancangan_akhir_renja_sub_kegiatan_data rsub')
            ->join('renstra_sub_kegiatan rsk', 'rsk.id = rsub.sub_kegiatan_id')
            ->where('rsub.deleted_at IS NULL');
        if (!empty($KodeWilayah)) $this->db->where('rsub.kode_wilayah', $KodeWilayah);
        if (!empty($tahun) && $tahun !== 'all') $this->db->where('rsub.tahun', $tahun);
        if (!empty($filter_instansi)) $this->db->where('rsub.id_instansi', (int)$filter_instansi);
        $subDataRows = $this->db->get()->result_array();

        if (empty($subDataRows)) {
            $this->db->select('rsk.kode_nomenklatur, rsub.kab_kota, rsub.kecamatan, rsub.desa, rsub.lokasi_pelaksanaan, rsub.sumber_dana')
                ->from('renja_sub_kegiatan_data rsub')
                ->join('renstra_sub_kegiatan rsk', 'rsk.id = rsub.sub_kegiatan_id')
                ->where('rsub.deleted_at IS NULL');
            if (!empty($KodeWilayah)) $this->db->where('rsub.kode_wilayah', $KodeWilayah);
            if (!empty($tahun) && $tahun !== 'all') $this->db->where('rsub.tahun', $tahun);
        ...
        if (empty($subDataRows)) {
            $this->db->select('rsk.kode_nomenklatur, rsub.kab_kota, rsub.kecamatan, rsub.desa, rsub.lokasi_pelaksanaan, rsub.sumber_dana')
                ->from('rancangan_renja_sub_kegiatan_data rsub')
                ->join('renstra_sub_kegiatan rsk', 'rsk.id = rsub.sub_kegiatan_id')
                ->where('rsub.deleted_at IS NULL');
            if (!empty($KodeWilayah)) $this->db->where('rsub.kode_wilayah', $KodeWilayah);
            if (!empty($tahun) && $tahun !== 'all') $this->db->where('rsub.tahun', $tahun);
        ...
        foreach ($subDataRows as $sdr) {
            $skKode = trim($sdr['kode_nomenklatur'] ?? '');
            if (!$skKode) continue;
            $kab = trim($sdr['kab_kota'] ?? '');
            $kec = trim($sdr['kecamatan'] ?? '');
            $desa = trim($sdr['desa'] ?? '');
            $lok = trim($sdr['lokasi_pelaksanaan'] ?? '');

            $lokText = '';
            if ($desa && $kec && $kab) {
                $lokText = "$kab, Kec. $kec, $desa";
            } elseif ($kec && $kab) {
                $lokText = "$kab, Kec. $kec";
            } elseif ($kab) {
                $lokText = $kab;
            ...
            if ($lokText) {
                $rankhirLokasiMap[$skKode] = $lokText;
            }
            if (!empty($sdr['sumber_dana'])) {
                $rankhirSumberDanaMap[$skKode] = $sdr['sumber_dana'];
            }
        }

        // Proses dari Belanja Sub Kegiatan Header
        if (!empty($headers)) {
            ...
            foreach ($headers as $h) {
                ...
                $existingSubkeg[$subKegKode] = true;

                $lokasiSubKeg = isset($rankhirLokasiMap[$subKegKode]) ? $rankhirLokasiMap[$subKegKode] : ($namaWilayah . ', Semua Kecamatan, Semua Kelurahan/Desa');
                $sumberDanaSubKeg = isset($rankhirSumberDanaMap[$subKegKode]) ? $rankhirSumberDanaMap[$subKegKode] : 'Pendapatan Asli Daerah (PAD)';
                ...
                foreach ($hReks as $rek) {
                    ...
                    foreach ($rekRincs as $rinc) {
                        ...
                        $rList[] = [
                            ...
                            'sumberDana' => $sumberDanaSubKeg ?: ($rinc['sumber_dana'] ?: 'Pendapatan Asli Daerah (PAD)'),
                            'lokasi' => $lokasiSubKeg,
                ...
                $subKegData = [
                    ...
                    'perangkatDaerah' => $h['nama_perangkat_daerah'] ?: ($instRow ? $instRow['nama'] : ''),
                    'sumberDana' => $sumberDanaSubKeg ?: (count($rList) > 0 && !empty($rList[0]['sumberDana']) ? $rList[0]['sumberDana'] : 'Pendapatan Asli Daerah (PAD)'),
                    'lokasi' => $lokasiSubKeg,
    ...
    public function SaveTargetRenaksiSubKegiatan() {
        ...
        try {
            ...
            if (!empty($anggaran) && is_array($anggaran)) {
                ...
                if (!$existAng && !empty($kode_sub_kegiatan)) {
                    $this->db
                        ->where('deleted_at IS NULL')
                    ...
                    $existAng = $this->db->get('target_renaksi_anggaran_bulanan')->row_array();
                }
    ...
    public function SyncDPAAnggaran() {
        ...
        try {
            ...
            $this->db->select('*')->from('dpa_rak_rincian')->where('deleted_at IS NULL')->where('tahun', $tahun);
            if (!empty($instansi_id)) {
                $this->db->group_start()
                    ->where('id_instansi', (int)$instansi_id)
                    ->or_where('id_instansi IS NULL')
                    ->or_where('id_instansi', 0)
                ->group_end();
            }
            if (!empty($kode_wilayah)) {
                $this->db->where('kode_wilayah', $kode_wilayah);
            }
            $allRakRows = $this->db->order_by('id', 'ASC')->get()->result_array();

            $uniqueRakRows = [];
            foreach ($allRakRows as $r) {
                $rKey = '';
                if (!empty($r['rincian_id'])) {
                    $rKey = 'r_' . $r['rincian_id'];
                } else {
                    $rKey = 'k_' . $normalizeCode($r['kode_sub_kegiatan']) . '_' . trim($r['kode_rekening'] ?? '') . '_' . trim($r['uraian'] ?? '');
                }
                $uniqueRakRows[$rKey] = $r;
            }

            $rakByNormCode = [];
            foreach ($uniqueRakRows as $r) {
                $rSk = trim($r['kode_sub_kegiatan'] ?? '');
                $rNorm = $normalizeCode($rSk);
                if (empty($rNorm)) continue;

                if (!isset($rakByNormCode[$rNorm])) {
                    $rakByNormCode[$rNorm] = [
                ...
                $rakByNormCode[$rNorm]['jan'] += (float)($r['jan'] ?? 0);
                $rakByNormCode[$rNorm]['feb'] += (float)($r['feb'] ?? 0);
                $rakByNormCode[$rNorm]['mar'] += (float)($r['mar'] ?? 0);
                $rakByNormCode[$rNorm]['apr'] += (float)($r['apr'] ?? 0);
                $rakByNormCode[$rNorm]['mei'] += (float)($r['mei'] ?? 0);
                $rakByNormCode[$rNorm]['jun'] += (float)($r['jun'] ?? 0);
                $rakByNormCode[$rNorm]['jul'] += (float)($r['jul'] ?? 0);
                $rakByNormCode[$rNorm]['ags'] += (float)($r['ags'] ?? ($r['agu'] ?? 0));
                $rakByNormCode[$rNorm]['sep'] += (float)($r['sep'] ?? 0);
                $rakByNormCode[$rNorm]['okt'] += (float)($r['okt'] ?? 0);
                $rakByNormCode[$rNorm]['nov'] += (float)($r['nov'] ?? 0);
                $rakByNormCode[$rNorm]['des'] += (float)($r['des'] ?? 0);

                $mTot = (float)($r['jan'] ?? 0) + (float)($r['feb'] ?? 0) + (float)($r['mar'] ?? 0) + (float)($r['apr'] ?? 0) + 
                        (float)($r['mei'] ?? 0) + (float)($r['jun'] ?? 0) + (float)($r['jul'] ?? 0) + (float)($r['ags'] ?? ($r['agu'] ?? 0)) + 
                        (float)($r['sep'] ?? 0) + (float)($r['okt'] ?? 0) + (float)($r['nov'] ?? 0) + (float)($r['des'] ?? 0);
                $rakByNormCode[$rNorm]['total'] += (float)($r['total_rak'] ?? 0) > 0 ? (float)$r['total_rak'] : $mTot;
            }
            ...
            foreach ($headers as $h) {
                ...
                $subNorm = $normalizeCode($subKode);

                $rak = isset($rakByNormCode[$subNorm]) ? $rakByNormCode[$subNorm] : null;

                if ($rak && $rak['total'] > 0) {
                    $m0 = $rak['jan']; $m1 = $rak['feb']; $m2 = $rak['mar']; $m3 = $rak['apr'];
                    $m4 = $rak['mei']; $m5 = $rak['jun']; $m6 = $rak['jul']; $m7 = $rak['ags'];
                    $m8 = $rak['sep']; $m9 = $rak['okt']; $m10 = $rak['nov']; $m11 = $rak['des'];
                    $totalRak = $rak['total'];
                } else {
                    $totalRak = (float)$h['total_belanja'];
                    $m0 = $m1 = $m2 = $m3 = $m4 = $m5 = $m6 = $m7 = $m8 = $m9 = $m10 = $m11 = 0;
                }
                ...
                $angData = [
                    ...
                    'jan' => $m0, 'feb' => $m1, 'mar' => $m2, 'apr' => $m3,
                    'mei' => $m4, 'jun' => $m5, 'jul' => $m6, 'ags' => $m7,
                    'sep' => $m8, 'okt' => $m9, 'nov' => $m10, 'des' => $m11,
                    'total_anggaran' => $totalRak,
                    'created_at' => $now,
                    'updated_at' => $now
                ];
    ...
    private function _buildTargetRenaksiHierarchy($KodeWilayah, $tahun, $filter_instansi = null) {
        ...
        foreach ($dpaHeaders as $dh) {
            $dhSk = trim($dh['kode_sub_kegiatan']);
            $dpaHeaderMap[(int)$dh['id']] = $dh;
            $dpaHeaderBySubkeg[$dhSk] = $dh;
            $dpaHeaderBySubkeg['norm_' . $normalizeCode($dhSk)] = $dh;
            $headerIds[] = (int)$dh['id'];
        }
        ...
        $dpaRakRaw = $this->db->order_by('id', 'ASC')->get()->result_array();

        // Deduplikasi entri rincian RAK agar rincian yang sama tidak dijumlahkan berulang kali
        $uniqueRak = [];
        foreach ($dpaRakRaw as $dr) {
            $rKey = '';
            if (!empty($dr['rincian_id'])) {
                $rKey = 'r_' . $dr['rincian_id'];
            } else {
                $rKey = 'k_' . $normalizeCode($dr['kode_sub_kegiatan']) . '_' . trim($dr['kode_rekening'] ?? '') . '_' . trim($dr['uraian'] ?? '');
            }
            $uniqueRak[$rKey] = $dr;
        }

        $dpaRakMap = [];
        foreach ($uniqueRak as $dr) {
            $sk = trim($dr['kode_sub_kegiatan'] ?? '');
            if (empty($sk)) continue;

            $skNorm = 'norm_' . $normalizeCode($sk);
            ...
            $mJun = (float)($dr['jun'] ?? 0);
            $mJul = (float)($dr['jul'] ?? 0);
            $mAgu = (float)($dr['ags'] ?? ($dr['agu'] ?? 0));
            $mSep = (float)($dr['sep'] ?? 0);
            $mOkt = (float)($dr['okt'] ?? 0);
            $mNov = (float)($dr['nov'] ?? 0);
            $mDes = (float)($dr['des'] ?? 0);
            $mTot = $mJan + $mFeb + $mMar + $mApr + $mMei + $mJun + $mJul + $mAgu + $mSep + $mOkt + $mNov + $mDes;
            $rowTot = (float)($dr['total_rak'] ?? 0) > 0 ? (float)$dr['total_rak'] : $mTot;

            $dpaRakMap[$sk]['jan'] += $mJan;
            $dpaRakMap[$sk]['feb'] += $mFeb;
            $dpaRakMap[$sk]['mar'] += $mMar;
            $dpaRakMap[$sk]['apr'] += $mApr;
            $dpaRakMap[$sk]['mei'] += $mMei;
            $dpaRakMap[$sk]['jun'] += $mJun;
            $dpaRakMap[$sk]['jul'] += $mJul;
            $dpaRakMap[$sk]['agu'] += $mAgu;
            $dpaRakMap[$sk]['sep'] += $mSep;
            $dpaRakMap[$sk]['okt'] += $mOkt;
            $dpaRakMap[$sk]['nov'] += $mNov;
            $dpaRakMap[$sk]['des'] += $mDes;
            $dpaRakMap[$sk]['total'] += $rowTot;

            $dpaRakMap[$skNorm]['jan'] += $mJan;
            $dpaRakMap[$skNorm]['feb'] += $mFeb;
            $dpaRakMap[$skNorm]['mar'] += $mMar;
            $dpaRakMap[$skNorm]['apr'] += $mApr;
            $dpaRakMap[$skNorm]['mei'] += $mMei;
            $dpaRakMap[$skNorm]['jun'] += $mJun;
            $dpaRakMap[$skNorm]['jul'] += $mJul;
            $dpaRakMap[$skNorm]['agu'] += $mAgu;
            $dpaRakMap[$skNorm]['sep'] += $mSep;
            $dpaRakMap[$skNorm]['okt'] += $mOkt;
            $dpaRakMap[$skNorm]['nov'] += $mNov;
            $dpaRakMap[$skNorm]['des'] += $mDes;
            $dpaRakMap[$skNorm]['total'] += $rowTot;

            // Alias keys
            $dpaRakMap['c_' . $sk] = $dpaRakMap[$sk];

            if (!empty($dr['header_id'])) {
                $hKey = 'h_' . $dr['header_id'];
                if (!isset($dpaRakMap[$hKey])) {
                    $dpaRakMap[$hKey] = [
                        'jan' => 0, 'feb' => 0, 'mar' => 0, 'apr' => 0,
                        'mei' => 0, 'jun' => 0, 'jul' => 0, 'agu' => 0,
                        'sep' => 0, 'okt' => 0, 'nov' => 0, 'des' => 0,
                ...
                $dpaRakMap[$hKey]['jan'] += $mJan;
                $dpaRakMap[$hKey]['feb'] += $mFeb;
                $dpaRakMap[$hKey]['mar'] += $mMar;
                $dpaRakMap[$hKey]['apr'] += $mApr;
                $dpaRakMap[$hKey]['mei'] += $mMei;
                $dpaRakMap[$hKey]['jun'] += $mJun;
                $dpaRakMap[$hKey]['jul'] += $mJul;
                $dpaRakMap[$hKey]['agu'] += $mAgu;
                $dpaRakMap[$hKey]['sep'] += $mSep;
                $dpaRakMap[$hKey]['okt'] += $mOkt;
                $dpaRakMap[$hKey]['nov'] += $mNov;
                $dpaRakMap[$hKey]['des'] += $mDes;
                $dpaRakMap[$hKey]['total'] += $rowTot;
            }
        }
        ...
        foreach ($anggaranSaved as $ang) {
            ...
            if (!empty($ang['kode_sub_kegiatan'])) {
                $angSk = trim($ang['kode_sub_kegiatan']);
                $anggaranMap['c_' . $angSk] = $val;
                $anggaranMap['norm_' . $normalizeCode($angSk)] = $val;
            }
        }
                ...
                foreach ($progRows as $p) {
                    ...
                    foreach ($kegRows as $k) {
                        ...
                        foreach ($subRows as $skItem) {
                            ...
                            $subNorm = 'norm_' . $normalizeCode($subKode);
                            $dpaH = isset($dpaHeaderBySubkeg[$subKode]) ? $dpaHeaderBySubkeg[$subKode] : (isset($dpaHeaderBySubkeg[$subNorm]) ? $dpaHeaderBySubkeg[$subNorm] : null);
                            ...
                            // Anggaran Menarik dari DPA Belanja Sub Kegiatan / RAK
                            $dpaRakItem = null;
                            if (!empty($hId) && isset($dpaRakMap['h_' . $hId])) {
                                $dpaRakItem = $dpaRakMap['h_' . $hId];
                            } else if (isset($dpaRakMap[$subKode])) {
                                $dpaRakItem = $dpaRakMap[$subKode];
                            } else if (isset($dpaRakMap['c_' . $subKode])) {
                                $dpaRakItem = $dpaRakMap['c_' . $subKode];
                            } else if (isset($dpaRakMap[$subNorm])) {
                                $dpaRakItem = $dpaRakMap[$subNorm];
                            }

                            $angItem = null;
                            if (!empty($hId) && isset($anggaranMap['h_' . $hId])) {
                                $angItem = $anggaranMap['h_' . $hId];
                            } else if (isset($anggaranMap['c_' . $subKode])) {
                                $angItem = $anggaranMap['c_' . $subKode];
                            } else if (isset($anggaranMap[$subNorm])) {
                                $angItem = $anggaranMap[$subNorm];
                            }

                            $totalBelanjaSub = 0;
                            if ($dpaRakItem && $dpaRakItem['total'] > 0) {
                                $totalBelanjaSub = (float)$dpaRakItem['total'];
                            }
                            if ($dpaH) {
                                $dpaTot = (float)$dpaH['total_belanja'];
                                if ($dpaTot <= 0 && isset($rekTotalMap[(int)$dpaH['id']]) && $rekTotalMap[(int)$dpaH['id']] > 0) {
                                    $dpaTot = $rekTotalMap[(int)$dpaH['id']];
                                }
                                if ($dpaTot > 0) {
                                    $totalBelanjaSub = $dpaTot;
                                }
                            }
                            ...
                            if ($angItem) {
                                $pagu = $angItem['pagu'];
                                if ($totalAnggaran <= 0) $totalAnggaran = $angItem['total'];
                            } else if ($dpaRakItem) {
                                $pagu = [
                                    'jan' => (float)$dpaRakItem['jan'], 'feb' => (float)$dpaRakItem['feb'], 'mar' => (float)$dpaRakItem['mar'],
                                    'apr' => (float)$dpaRakItem['apr'], 'mei' => (float)$dpaRakItem['mei'], 'jun' => (float)$dpaRakItem['jun'],
                                    'jul' => (float)$dpaRakItem['jul'], 'agu' => (float)$dpaRakItem['agu'], 'sep' => (float)$dpaRakItem['sep'],
                                    'okt' => (float)$dpaRakItem['okt'], 'nov' => (float)$dpaRakItem['nov'], 'des' => (float)$dpaRakItem['des']
                                ];
                                if ($totalAnggaran <= 0) $totalAnggaran = $dpaRakItem['total'];
                            } else if ($totalAnggaran > 0) {
                                $mShare = $totalAnggaran / 12;
                                $pagu = [
                                    'jan' => $mShare, 'feb' => $mShare, 'mar' => $mShare, 'apr' => $mShare,
                                    'mei' => $mShare, 'jun' => $mShare, 'jul' => $mShare, 'agu' => $mShare,
                                    'sep' => $mShare, 'okt' => $mShare, 'nov' => $mShare, 'des' => $mShare
                                ];
    ...
    public function GetSubKegiatanRealisasiDetail() {
        ...
        // 1. Target Anggaran Bulanan dari Target Renaksi
        $targetAng = null;
        if ($hId > 0) {
        ...
        if (!$targetAng && !empty($subKode)) {
            $this->db->where('deleted_at IS NULL')
                ->where('kode_sub_kegiatan', $subKode)
                ->where('tahun', $thn);
            if (!empty($header['id_instansi'])) {
                $this->db->where('id_instansi', (int)$header['id_instansi']);
            }
            $targetAng = $this->db->get('target_renaksi_anggaran_bulanan')->row_array();
        }

        if (!$targetAng) {
            // Ambil dari DPA RAK rincian jika belum tersimpan di target_renaksi_anggaran_bulanan
            $this->db->where('tahun', $thn)->where('deleted_at IS NULL');
            if ($hId > 0) {
                $this->db->where('header_id', $hId);
            } else if (!empty($subKode)) {
                $this->db->where('kode_sub_kegiatan', $subKode);
                if (!empty($header['id_instansi'])) {
                    $this->db->where('id_instansi', (int)$header['id_instansi']);
                }
            }
            $rakRows = $this->db->get('dpa_rak_rincian')->result_array();
            ...
            if (!empty($rakRows)) {
                ...
                foreach ($rakRows as $r) {
                    $targetAng['jan'] += (float)$r['jan'];
                    $targetAng['feb'] += (float)$r['feb'];
                    $targetAng['mar'] += (float)$r['mar'];
                    $targetAng['apr'] += (float)$r['apr'];
                    $targetAng['mei'] += (float)$r['mei'];
                    $targetAng['jun'] += (float)$r['jun'];
                    $targetAng['jul'] += (float)$r['jul'];
                    $targetAng['ags'] += (float)($r['ags'] ?? ($r['agu'] ?? 0));
                    $targetAng['sep'] += (float)$r['sep'];
                    $targetAng['okt'] += (float)$r['okt'];
                    $targetAng['nov'] += (float)$r['nov'];
                    $targetAng['des'] += (float)$r['des'];
                    $targetAng['total_anggaran'] += (float)$r['total_rak'];
                }
            } else if (!empty($header['total_belanja'])) {
                $mShare = (float)$header['total_belanja'] / 12;
                $targetAng = [
                    'jan' => $mShare, 'feb' => $mShare, 'mar' => $mShare, 'apr' => $mShare,
                    'mei' => $mShare, 'jun' => $mShare, 'jul' => $mShare, 'ags' => $mShare,
                    'sep' => $mShare, 'okt' => $mShare, 'nov' => $mShare, 'des' => $mShare,
                    'total_anggaran' => (float)$header['total_belanja']
                ];
            }
        }
        ...
        if (!$realAng && !empty($subKode)) {
            ...
            $realAng = $this->db->get('realisasi_renaksi_anggaran_bulanan')->row_array();
        }
        ...
        for ($m = 0; $m < 12; $m++) {
            $k = $monthKeys[$m];
            $tVal = $targetAng ? (float)(isset($targetAng[$k]) ? $targetAng[$k] : ($k === 'ags' && isset($targetAng['agu']) ? $targetAng['agu'] : 0)) : 0;
            $rVal = ($realAng && isset($realAng[$k]) && $realAng[$k] !== null && $realAng[$k] !== '') ? (float)$realAng[$k] : null;
            if ($rVal !== null) {
                $runningReal += $rVal;
            }
            $bulananList[] = [
        ...
        try {
            ...
            if (!empty($bulananReal) && is_array($bulananReal)) {
                ...
                if (!$existAng && !empty($kode_sub_kegiatan)) {
                    ...
                    $existAng = $this->db->get('realisasi_renaksi_anggaran_bulanan')->row_array();
                }
    ...
    private function _buildRealisasiRenaksiHierarchy($KodeWilayah, $tahun, $filter_instansi, $bulanIdx) {
        ...
        // 1. Ambil seluruh pohon Target Renaksi (Tujuan -> SS -> Program -> Kegiatan -> Sub Kegiatan)
        $targetTree = $this->_buildTargetRenaksiHierarchy($KodeWilayah, $tahun, $filter_instansi);
        ...
        // 4. Ekstrak Program -> Kegiatan -> Sub Kegiatan dari pohon Target Renaksi
        $programs = [];

        foreach (($targetTree['tujuanList'] ?? []) as $t) {
            foreach (($t['sasaranStrategisList'] ?? []) as $ss) {
                foreach (($ss['sasaranProgramList'] ?? []) as $sp) {
                    $pKode = $sp['kode'];
                    $pName = $sp['nomenklatur'] ?: $sp['nama'];
                    $pPerihal = $sp['nama'];

                    // Indikator Program Kinerja
                    $pInd = !empty($sp['indikators'][0]) ? $sp['indikators'][0] : [
                        'id' => 0, 'nama' => 'Indikator ' . $pName, 'satuan' => '%', 'targetTahunan' => 100,
                        'tw1' => null, 'tw2' => null, 'tw3' => null, 'tw4' => null
                    ];
                    $rProgKin = isset($realHierMap['program_' . $pKode]) ? $realHierMap['program_' . $pKode] : [null, null, null, null];
                    $progKinTW = [];
                    for ($tIdx = 0; $tIdx < 4; $tIdx++) {
                        $twNum = $tIdx + 1;
                        $progKinTW[] = [
                            'target' => isset($pInd['tw' . $twNum]) && $pInd['tw' . $twNum] !== null ? (float)$pInd['tw' . $twNum] : null,
                            'realisasi' => $rProgKin[$tIdx]
                        ];
                    }

                    $kegList = [];
                    foreach (($sp['sasaranKegiatanList'] ?? []) as $sk) {
                        $kKode = $sk['kode'];
                        $kName = $sk['nomenklatur'] ?: $sk['nama'];
                        $kPerihal = $sk['perihal'] ?: $sk['nama'];

                        // Indikator Kegiatan Kinerja
                        $kInd = !empty($sk['indikators'][0]) ? $sk['indikators'][0] : [
                            'id' => 0, 'nama' => 'Indikator ' . $kName, 'satuan' => '%', 'targetTahunan' => 100,
                            'tw1' => null, 'tw2' => null, 'tw3' => null, 'tw4' => null
                        ];
                        $rKegKin = isset($realHierMap['kegiatan_' . $kKode]) ? $realHierMap['kegiatan_' . $kKode] : [null, null, null, null];
                        $kegKinTW = [];
                        for ($tIdx = 0; $tIdx < 4; $tIdx++) {
                            $twNum = $tIdx + 1;
                            $kegKinTW[] = [
                                'target' => isset($kInd['tw' . $twNum]) && $kInd['tw' . $twNum] !== null ? (float)$kInd['tw' . $twNum] : null,
                                'realisasi' => $rKegKin[$tIdx]
                            ];
                        }

                        $subList = [];
                        foreach (($sk['sasaranSubKegiatanList'] ?? []) as $sub) {
                            $hId = isset($sub['headerId']) ? (int)$sub['headerId'] : (int)$sub['id'];
                            $subKode = $sub['kode'];
                            $subName = $sub['nomenklatur'] ?: $sub['nama'];
                            $subPerihal = $sub['perihal'] ?: $sub['nama'];

                            // Target Anggaran Bulanan dari Target Renaksi
                            $pagu = $sub['paguBulanan'] ?? [];
                            ...
                            for ($m = 0; $m < 12; $m++) {
                                $mk = $monthKeys[$m];
                                $tM = isset($pagu[$mk]) ? (float)$pagu[$mk] : (isset($pagu['agu']) && $mk === 'ags' ? (float)$pagu['agu'] : 0);
                                $rM = ($rAngRow && isset($rAngRow[$mk]) && $rAngRow[$mk] !== null && $rAngRow[$mk] !== '') ? (float)$rAngRow[$mk] : null;
                                if ($rM !== null) {
                                    $runningReal += $rM;
                                }
                            ...
                            // Indikator Sub Kegiatan
                            $subIndikatorData = [];
                            foreach (($sub['indikators'] ?? []) as $sInd) {
                                $sIndId = $sInd['id'];
                                $rTw = isset($realIndMap[$sIndId]) ? $realIndMap[$sIndId] : [null, null, null, null];
                                $twObjList = [];
                                for ($tIdx = 0; $tIdx < 4; $tIdx++) {
                                    $twNum = $tIdx + 1;
                                    $twObjList[] = [
                                        'target' => isset($sInd['tw' . $twNum]) && $sInd['tw' . $twNum] !== null ? (float)$sInd['tw' . $twNum] : null,
                                        'realisasi' => $rTw[$tIdx]
                                    ];
                                }
                                $subIndikatorData[] = [
                                    ...
                                    'uraian' => $sInd['nama'],
                                    'satuan' => $sInd['satuan'],
                                    'targetTahunan' => $sInd['targetTahunan'],
                                    'tw' => $twObjList
                                ];
                            }
                            ...
                            $subList[] = [
                                ...
                                'nomenklatur' => $subName,
                                'perihal' => $subPerihal,
                                'perangkatDaerah' => $sub['perangkatDaerah'] ?? '',
                                'subUnit' => $sub['subUnit'] ?? '',
                                'bidangUrusan' => $sub['bidangUrusan'] ?? '',
                                'programNama' => $sub['programNama'] ?? $pName,
                                'kegiatanNama' => $sub['kegiatanNama'] ?? $kName,
                                'bulanan' => $subBulanan,
                                'targetAnggaran' => (float)$sub['anggaran'],
    ...
    public function LaporanAnggaran() {
        ...
        // Ambil Data Laporan Anggaran bersumber dari Sub Kegiatan Realisasi Renaksi
        if (!$is_logged_in && empty($KodeWilayah)) {
            $Data['Items'] = [];
            $Data['Summary'] = [
                'total_murni' => 0,
            ...
            $laporanData = $this->_buildLaporanAnggaranData($KodeWilayah, $tahun, $filter_instansi, $bulan_sampai);
            $Data['Items'] = $laporanData['items'];
            $Data['Summary'] = $laporanData['summary'];
        }
    ...
    public function GetLaporanAnggaranData() {
        ...
        echo json_encode([
            'status' => 'success',
            'data' => [
                'items' => $laporanData['items'],
                'summary' => $laporanData['summary']
            ]
        ]);
    }

    private function _buildLaporanAnggaranData($KodeWilayah, $tahun, $filter_instansi, $bulan_sampai) {
        if ($bulan_sampai < 0) $bulan_sampai = 0;
        if ($bulan_sampai > 11) $bulan_sampai = 11;

        // 1. Ambil hierarki Realisasi Renaksi (Program -> Kegiatan -> Sub Kegiatan)
        $tree = $this->_buildRealisasiRenaksiHierarchy($KodeWilayah, $tahun, $filter_instansi, $bulan_sampai);

        // 2. Ambil data overrides dari laporan_anggaran_rekap jika ada
        $this->db->where('deleted_at IS NULL');
        if (!empty($tahun) && $tahun !== 'all') {
        ...
        $savedRekap = $this->db->get('laporan_anggaran_rekap')->result_array();
        $savedByHeaderId = [];
        $savedByCode = [];
        foreach ($savedRekap as $sr) {
            if (!empty($sr['header_id'])) {
                $savedByHeaderId[(int)$sr['header_id']] = $sr;
            }
            if (!empty($sr['kode_rekening'])) {
                $savedByCode[trim($sr['kode_rekening'])] = $sr;
            }
        }

        // 3. Ekstrak seluruh sub kegiatan dari tree Realisasi Renaksi
        $items = [];
        $seenSubCodes = [];
        $urutan = 1;

        foreach ($tree as $prog) {
            $pName = $prog['nomenklatur'] ?: $prog['nama'];
            foreach ($prog['kegiatan'] as $keg) {
                $kName = $keg['nomenklatur'] ?: $keg['nama'];
                foreach ($keg['subKegiatan'] as $sub) {
                    $hId = (int)($sub['headerId'] ?? ($sub['id'] ?? 0));
                    $subKode = trim($sub['kode'] ?? '');
                    if (empty($subKode) && $hId <= 0) continue;

                    $subKey = $hId > 0 ? ('h_' . $hId) : ('c_' . $subKode);
                    if (isset($seenSubCodes[$subKey])) continue;
                    $seenSubCodes[$subKey] = true;

                    $subName = $sub['nomenklatur'] ?: $sub['nama'];
                    $targetAnggaran = (float)($sub['targetAnggaran'] ?? 0);
                    $realisasiAnggaran = (float)($sub['realisasiAnggaran'] ?? 0);

                    // Cek jika ada data tersimpan di laporan_anggaran_rekap
                    $savedRow = null;
                    if ($hId > 0 && isset($savedByHeaderId[$hId])) {
                        $savedRow = $savedByHeaderId[$hId];
                    } else if (!empty($subKode) && isset($savedByCode[$subKode])) {
                        $savedRow = $savedByCode[$subKode];
                    }

                    if ($savedRow) {
                        $rowId = (int)$savedRow['id'];
                        $anggaranMurni = ((float)$savedRow['anggaran_murni'] > 0) ? (float)$savedRow['anggaran_murni'] : $targetAnggaran;
                        $anggaranPerubahan = ((float)$savedRow['anggaran_perubahan'] > 0) ? (float)$savedRow['anggaran_perubahan'] : $targetAnggaran;
                        // Prioritaskan realisasi dari renaksi jika ada atau gunakan dari savedRow
                        $realisasi = ($realisasiAnggaran > 0 || (isset($sub['realisasiAnggaran']) && $sub['realisasiAnggaran'] !== null)) ? $realisasiAnggaran : (float)$savedRow['realisasi_anggaran'];
                    } else {
                        $rowId = $hId; // Default row identifier
                        $anggaranMurni = $targetAnggaran;
                        $anggaranPerubahan = $targetAnggaran;
                        $realisasi = $realisasiAnggaran;
                    }

                    $sisa = $anggaranPerubahan - $realisasi;
                    $persen = ($anggaranPerubahan > 0) ? round(($realisasi / $anggaranPerubahan) * 100, 2) : 0;
                    $status = ($anggaranPerubahan >= $anggaranMurni) ? '+' : '-';

                    $items[] = [
                        'id' => $rowId,
                        'header_id' => $hId,
                        'kode_rekening' => $subKode,
                        'nama_sub_kegiatan' => $subName,
                        'anggaran_murni' => $anggaranMurni,
                        'anggaran_perubahan' => $anggaranPerubahan,
                        'perubahan_status' => $status,
                        'realisasi_anggaran' => $realisasi,
                        'sisa_anggaran' => $sisa,
                        'persen_capaian' => $persen,
                        'bulan_sampai' => $bulan_sampai,
                        'urutan' => $urutan++,
                        'program_nama' => $sub['programNama'] ?? $pName,
                        'kegiatan_nama' => $sub['kegiatanNama'] ?? $kName,
                        'perangkat_daerah' => $sub['perangkatDaerah'] ?? ''
                    ];
                }
            }
        }

        // Fallback: Jika tree kosong, coba ambil langsung dari belanja_sub_kegiatan_header
        if (empty($items)) {
            $this->db->select('*')->from('belanja_sub_kegiatan_header')->where('deleted_at IS NULL');
            if (!empty($KodeWilayah)) $this->db->where('kode_wilayah', $KodeWilayah);
            if (!empty($tahun)) $this->db->where('tahun', $tahun);
            if (!empty($filter_instansi)) $this->db->where('id_instansi', (int)$filter_instansi);
            $rawHeaders = $this->db->order_by('kode_sub_kegiatan', 'ASC')->get()->result_array();
            ...
            foreach ($rawHeaders as $rh) {
                $hId = (int)$rh['id'];
                $subKode = trim($rh['kode_sub_kegiatan']);
                $subName = $rh['nama_sub_kegiatan'];
                $totalBelanja = (float)$rh['total_belanja'];

                // Realisasi dari bulanan
                $realRow = $this->db->where('deleted_at IS NULL')
                    ->group_start()
                        ->where('header_id', $hId)
                        ->or_where('kode_sub_kegiatan', $subKode)
                    ->group_end()
                    ->where('tahun', $tahun)
                    ->get('realisasi_renaksi_anggaran_bulanan')
                    ->row_array();
                ...
                if ($realRow) {
                    foreach ($selectedMonths as $mc) {
                        $realisasi += (float)($realRow[$mc] ?? 0);
                    }
                }

                $savedRow = isset($savedByHeaderId[$hId]) ? $savedByHeaderId[$hId] : (isset($savedByCode[$subKode]) ? $savedByCode[$subKode] : null);
                $anggaranMurni = $savedRow ? (float)$savedRow['anggaran_murni'] : $totalBelanja;
                $anggaranPerubahan = $savedRow ? (float)$savedRow['anggaran_perubahan'] : $totalBelanja;
                if ($anggaranPerubahan <= 0) $anggaranPerubahan = $totalBelanja;
                if ($anggaranMurni <= 0) $anggaranMurni = $totalBelanja;
                ...
                $items[] = [
                    'id' => $savedRow ? (int)$savedRow['id'] : $hId,
                    'header_id' => $hId,
                    'kode_rekening' => $subKode,
                    'nama_sub_kegiatan' => $subName,
                    'anggaran_murni' => $anggaranMurni,
                    ...
                    'bulan_sampai' => $bulan_sampai,
                    'urutan' => $urutan++,
                    'program_nama' => $rh['nama_program'] ?? '',
                    'kegiatan_nama' => $rh['nama_kegiatan'] ?? '',
                    'perangkat_daerah' => $rh['nama_perangkat_daerah'] ?? ''
                ];
            }
        }
        ...
        foreach ($items as $it) {
            $totMurni += (float)$it['anggaran_murni'];
            $totPerubahan += (float)$it['anggaran_perubahan'];
            $totRealisasi += (float)$it['realisasi_anggaran'];
            $totSisa += (float)$it['sisa_anggaran'];
        }
    ...
    public function SaveLaporanAnggaranItem() {
        ...
        try {
            ...
            $header_id = (int)$this->input->post('header_id', TRUE);
            $kode_rekening = trim($this->input->post('kode_rekening', TRUE));
            $nama_sub_kegiatan = trim($this->input->post('nama_sub_kegiatan', TRUE));
            $tahun = (int)($this->input->post('tahun', TRUE) ?: date('Y'));
            $id_instansi = (int)$this->input->post('instansi_id', TRUE);
            $bulan_sampai = (int)($this->input->post('bulan_sampai', TRUE) ?: 7);
            ...
            // Cek apakah data sudah ada di laporan_anggaran_rekap
            $existing = null;
            if ($id > 0) {
                $existing = $this->db->where('id', $id)->get('laporan_anggaran_rekap')->row_array();
            }
            if (!$existing && $header_id > 0) {
                $existing = $this->db->where('header_id', $header_id)->where('deleted_at IS NULL')->get('laporan_anggaran_rekap')->row_array();
            }
            if (!$existing && !empty($kode_rekening) && $tahun > 0) {
                $this->db->where('kode_rekening', $kode_rekening)->where('tahun', $tahun)->where('deleted_at IS NULL');
                if ($id_instansi > 0) $this->db->where('id_instansi', $id_instansi);
                $existing = $this->db->get('laporan_anggaran_rekap')->row_array();
            }
            ...
            if ($existing) {
                ...
                $this->db->where('id', $existing['id'])->update('laporan_anggaran_rekap', $updateData);
                $savedId = (int)$existing['id'];
            } else {
                $KodeWilayah = $this->get_kode_wilayah();
                $insertData = [
                    'header_id' => $header_id ?: ($id > 0 ? $id : null),
                    'kode_wilayah' => $KodeWilayah ?: '35.73',
                    'tahun' => $tahun,
                    'id_instansi' => $id_instansi ?: 1,
                ...
                $this->db->insert('laporan_anggaran_rekap', $insertData);
    ...
    public function SyncLaporanAnggaran() {
        ...
        try {
            ...
            $res = $this->_buildLaporanAnggaranData($KodeWilayah, $tahun, $filter_instansi, $bulan_sampai);
            $items = $res['items'];
            $syncedCount = 0;

            foreach ($items as $it) {
                $hId = (int)$it['header_id'];
                $code = $it['kode_rekening'];
                $nama = $it['nama_sub_kegiatan'];
                $murni = (float)$it['anggaran_murni'];
                $perubahan = (float)$it['anggaran_perubahan'];
                $realisasi = (float)$it['realisasi_anggaran'];
                $sisa = (float)$it['sisa_anggaran'];
                $persen = (float)$it['persen_capaian'];
                $status = $it['perubahan_status'];

                // Check existing in laporan_anggaran_rekap
                $existing = null;
                if ($hId > 0) {
                    $existing = $this->db->where('header_id', $hId)->where('deleted_at IS NULL')->get('laporan_anggaran_rekap')->row_array();
                }
                if (!$existing && !empty($code)) {
                    $this->db->where('kode_rekening', $code)->where('tahun', $tahun)->where('deleted_at IS NULL');
                    if (!empty($filter_instansi)) $this->db->where('id_instansi', (int)$filter_instansi);
                    $existing = $this->db->get('laporan_anggaran_rekap')->row_array();
                }

                if ($existing) {
                    $this->db->where('id', $existing['id'])->update('laporan_anggaran_rekap', [
                        'realisasi_anggaran' => $realisasi,
                        'sisa_anggaran' => $sisa,
                        'persen_capaian' => $persen,
                    ...
                    $this->db->insert('laporan_anggaran_rekap', [
                        'header_id' => $hId ?: null,
                        'kode_wilayah' => $KodeWilayah ?: '35.73',
                        'tahun' => $tahun,
                        'id_instansi' => (int)($filter_instansi ?: 1),
                        'kode_rekening' => $code,
            ...
            echo json_encode([
                'status' => 'success',
                'message' => "Berhasil menyinkronkan $syncedCount data Sub Kegiatan dari Realisasi Renaksi ke Laporan Anggaran!"
            ]);
        } catch (Exception $e) {
            echo json_encode(['status' => 'error', 'message' => $e->getMessage()]);
        }
    ...
    public function PengisianKebijakanStrategis() {
        ...
        $Data['filterInstansi'] = $activeInstansi;
        $Data['ActiveInstansiId'] = $activeInstansi;
        ...
        if ($activeInstansi) $this->db->where('instansi_id', (int)$activeInstansi);
        if (!$is_logged_in && empty($KodeWilayah)) {
            $Data['Items'] = [];
        } else {
    ...
    private function _ensure_cpk_table() {
        if (!$this->db->table_exists('lkpj_capaian_program_kegiatan')) {
            $this->db->query("CREATE TABLE IF NOT EXISTS `lkpj_capaian_program_kegiatan` (
              `id` INT(11) NOT NULL AUTO_INCREMENT,
              `kodewilayah` VARCHAR(20) DEFAULT '35.12',
              `instansi_id` INT(11) DEFAULT 1,
              `tahun` INT(4) NOT NULL DEFAULT 2026,
              `urusan` VARCHAR(255) DEFAULT 'Semua Urusan',
            ...
            if (!$this->db->field_exists('kode', 'lkpj_capaian_program_kegiatan')) {
                $this->db->query("ALTER TABLE `lkpj_capaian_program_kegiatan` ADD COLUMN `kode` VARCHAR(100) DEFAULT NULL AFTER `tipe`");
            }
        }
    }
    ...
    public function PengisianCapaianProgramKegiatan() {
        ...
        $this->_ensure_cpk_table();

        $KodeWilayah = $this->get_kode_wilayah();
        $instansi_id = $this->get_instansi_id();
        $is_role_4 = $this->is_role_4();
        $tahun = (int)($this->input->get('tahun', TRUE) ?: (isset($_SESSION['Tahun']) ? $_SESSION['Tahun'] : 2026));
        $filter_urusan = $this->input->get('urusan', TRUE) ?: 'Semua Urusan';
        $filter_instansi = $this->input->get('instansi_id', TRUE);
        if (empty($filter_instansi) && isset($_SESSION['TempInstansiId']) && !empty($_SESSION['TempInstansiId'])) {
            $filter_instansi = $_SESSION['TempInstansiId'];
        }
        ...
        // Ambil daftar Urusan yang tersedia
        $listUrusan = ['Semua Urusan'];
        $dbBidangUrusan = $this->db->select('DISTINCT(nama_bidang_urusan) as ur')
            ->where('deleted_at IS NULL')
            ->where('tahun', (int)$tahun);
        if (!empty($activeInstansi)) {
            $dbBidangUrusan->where('id_instansi', (int)$activeInstansi);
        }
        $bUrRows = $dbBidangUrusan->get('belanja_sub_kegiatan_header')->result_array();
        foreach ($bUrRows as $bu) {
            $urText = trim($bu['ur'] ?? '');
            if (!empty($urText) && !in_array($urText, $listUrusan)) {
                $listUrusan[] = $urText;
            }
        }

        if (!empty($KodeWilayah)) {
            $uPdRows = $this->db->select('nama_urusan')->where('kodewilayah', $KodeWilayah)->where('deleted_at IS NULL')->get('urusan_pd')->result_array();
            foreach ($uPdRows as $upd) {
                $urText = trim($upd['nama_urusan'] ?? '');
                if (!empty($urText) && !in_array($urText, $listUrusan)) {
                    $listUrusan[] = $urText;
                }
            }
        ...
        // Ambil Grouped Data otomatis dari Realisasi Renaksi & Kebijakan Sasaran Renstra
        if (!$is_logged_in && empty($KodeWilayah)) {
            $Data['Groups'] = [];
        } else {
            $Data['Groups'] = $this->_get_capaian_program_kegiatan_groups((int)$tahun, $filter_urusan, $activeInstansi);
        }
    ...
    private function _get_capaian_program_kegiatan_groups($tahun, $urusan, $instansi_id) {
        ...
        $instansi_id = (int)($instansi_id ?: ($this->get_instansi_id() ?: 1));
        $KodeWilayah = $this->get_kode_wilayah();
        if (empty($KodeWilayah)) {
            $instRow = $this->db->where('id', $instansi_id)->get('akun_instansi')->row_array();
            $KodeWilayah = ($instRow && !empty($instRow['kodewilayah'])) ? $instRow['kodewilayah'] : '35.12';
        }

        // 1. Ambil target tree dari Realisasi / Target Renaksi (Tujuan -> Sasaran Strategis -> Program -> Kegiatan -> Sub Kegiatan)
        $targetTree = $this->_buildTargetRenaksiHierarchy($KodeWilayah, $tahun, $instansi_id);

        // 2. Ambil realisasi anggaran bulanan dari realisasi_renaksi_anggaran_bulanan
        $this->db->where('deleted_at IS NULL');
        if (!empty($tahun) && $tahun !== 'all') $this->db->where('tahun', $tahun);
        if (!empty($instansi_id)) $this->db->where('id_instansi', $instansi_id);
        $realAngList = $this->db->get('realisasi_renaksi_anggaran_bulanan')->result_array();
        $realAngMap = [];
        $monthKeys = ['jan','feb','mar','apr','mei','jun','jul','ags','sep','okt','nov','des'];
        foreach ($realAngList as $ra) {
            $sumM = 0;
            foreach ($monthKeys as $mk) {
                if (isset($ra[$mk]) && $ra[$mk] !== null && $ra[$mk] !== '') {
                    $sumM += (float)$ra[$mk];
                }
            }
            if (!empty($ra['header_id'])) $realAngMap['h_' . $ra['header_id']] = $sumM;
            if (!empty($ra['kode_sub_kegiatan'])) $realAngMap['c_' . trim($ra['kode_sub_kegiatan'])] = $sumM;
        }

        // 3. Ambil realisasi indikator dari realisasi_renaksi_indikator
        $this->db->where('deleted_at IS NULL');
        if (!empty($tahun) && $tahun !== 'all') $this->db->where('tahun', $tahun);
        if (!empty($instansi_id)) $this->db->where('id_instansi', $instansi_id);
        $realInds = $this->db->get('realisasi_renaksi_indikator')->result_array();
        ...
        foreach ($realInds as $ri) {
            ...
            $k = $ri['level_type'] . '_' . trim($ri['entity_code']);
            $realHierMap[$k] = $twArr;
            if ($ri['indikator_id']) {
                $realHierMap['ind_' . $ri['indikator_id']] = $twArr;
            }
        }

        // 4. Ambil catatan evaluasi & override dari lkpj_capaian_program_kegiatan
        $this->db->where('deleted_at IS NULL');
        if ($tahun > 0) $this->db->where('tahun', $tahun);
        if ($instansi_id > 0) $this->db->where('instansi_id', $instansi_id);
        $savedRows = $this->db->order_by('urutan', 'ASC')->order_by('id', 'ASC')->get('lkpj_capaian_program_kegiatan')->result_array();
        
        $savedByTipeKode = [];
        $savedByUraian = [];
        $savedById = [];
        foreach ($savedRows as $sr) {
            $savedById[(int)$sr['id']] = $sr;
            $kodeKey = !empty($sr['kode']) ? trim($sr['kode']) : '';
            if ($kodeKey) {
                $savedByTipeKode[$sr['tipe'] . '_' . $kodeKey] = $sr;
            }
            $uraianKey = strtolower(trim($sr['uraian']));
            $savedByUraian[$sr['tipe'] . '_' . $uraianKey] = $sr;
        }

        // 5. Bangun Groups dari Pohon Renstra & Realisasi Renaksi
        $groups = [];
        $virtualIdCounter = 10000;
        $matchedSavedIds = [];

        foreach (($targetTree['tujuanList'] ?? []) as $t) {
            foreach (($t['sasaranStrategisList'] ?? []) as $ss) {
                $sasaranRenstra = $ss['nama'] ?: ('Sasaran ' . $ss['id']);

                foreach (($ss['sasaranProgramList'] ?? []) as $sp) {
                    $pKode = trim($sp['kode']);
                    $pName = $sp['nomenklatur'] ?: $sp['nama'];

                    // Bidang urusan / Urusan check
                    $pUrusan = 'Perencanaan';
                    if (!empty($sp['sasaranKegiatanList'][0]['sasaranSubKegiatanList'][0]['bidangUrusan'])) {
                        $pUrusan = $sp['sasaranKegiatanList'][0]['sasaranSubKegiatanList'][0]['bidangUrusan'];
                    }

                    // Filter Urusan jika dipilih
                    if (!empty($urusan) && $urusan !== 'Semua' && $urusan !== 'Semua Urusan') {
                        $matchedUrusan = false;
                        if (stripos($pUrusan, $urusan) !== false || stripos($urusan, $pUrusan) !== false) {
                            $matchedUrusan = true;
                        } else {
                            foreach (($sp['sasaranKegiatanList'] ?? []) as $skCheck) {
                                foreach (($skCheck['sasaranSubKegiatanList'] ?? []) as $subCheck) {
                                    if (!empty($subCheck['bidangUrusan']) && (stripos($subCheck['bidangUrusan'], $urusan) !== false || stripos($urusan, $subCheck['bidangUrusan']) !== false)) {
                                        $matchedUrusan = true;
                                        $pUrusan = $subCheck['bidangUrusan'];
                                        break 2;
                                    }
                                }
                            }
                    ...
                    $pTargetKin = isset($pInd['targetTahunan']) && (float)$pInd['targetTahunan'] > 0 ? (float)$pInd['targetTahunan'] : 100;
                    
                    // Realisasi Kinerja Program (TW4 atau nilai realisasi TW terakhir yang terisi)
                    $pRealKin = null;
                    $rProgKin = isset($realHierMap['program_' . $pKode]) ? $realHierMap['program_' . $pKode] : null;
                    if ($rProgKin) {
                        for ($tIdx = 3; $tIdx >= 0; $tIdx--) {
                            if ($rProgKin[$tIdx] !== null) {
                                $pRealKin = (float)$rProgKin[$tIdx];
                                break;
                            }
                        }
                    }
                    $pCapaianKin = ($pTargetKin > 0 && $pRealKin !== null) ? round(($pRealKin / $pTargetKin) * 100, 2) : 0;

                    // Saved evaluasi Program
                    $savedProg = isset($savedByTipeKode['program_' . $pKode]) ? $savedByTipeKode['program_' . $pKode] : 
                                (isset($savedByUraian['program_' . strtolower(trim($pName))]) ? $savedByUraian['program_' . strtolower(trim($pName))] : null);
                    ...
                    $progTinjut = '';
                    if ($savedProg) {
                        $progDbId = (int)$savedProg['id'];
                        $matchedSavedIds[$progDbId] = true;
                        $progPermasalahan = $savedProg['permasalahan'] ?? '';
                        $progUpaya = $savedProg['upaya'] ?? '';
                        $progTinjut = $savedProg['tinjut'] ?? '';
                    } else {
                        $virtualIdCounter++;
                        $progDbId = $virtualIdCounter;
                    }

                    // Kegiatan List
                    $kegiatanItems = [];
                    $progTotalAnggaran = 0;
                    $progTotalRealAnggaran = 0;
                    ...
                    foreach (($sp['sasaranKegiatanList'] ?? []) as $sk) {
                        ...
                        $kTargetKin = isset($kInd['targetTahunan']) && (float)$kInd['targetTahunan'] > 0 ? (float)$kInd['targetTahunan'] : 100;

                        // Realisasi Kinerja Kegiatan
                        $kRealKin = null;
                        $rKegKin = isset($realHierMap['kegiatan_' . $kKode]) ? $realHierMap['kegiatan_' . $kKode] : null;
                        if ($rKegKin) {
                            for ($tIdx = 3; $tIdx >= 0; $tIdx--) {
                                if ($rKegKin[$tIdx] !== null) {
                                    $kRealKin = (float)$rKegKin[$tIdx];
                                    break;
                                }
                            }
                        }
                        $kCapaianKin = ($kTargetKin > 0 && $kRealKin !== null) ? round(($kRealKin / $kTargetKin) * 100, 2) : 0;

                        // Anggaran & Realisasi Anggaran Kegiatan (dari Sub Kegiatan)
                        $kegAnggaran = 0;
                        $kegRealAnggaran = 0;
                        foreach (($sk['sasaranSubKegiatanList'] ?? []) as $sub) {
                            $subAng = (float)($sub['anggaran'] ?? ($sub['targetAnggaran'] ?? 0));
                            $kegAnggaran += $subAng;

                            $hId = isset($sub['headerId']) ? (int)$sub['headerId'] : (int)$sub['id'];
                            $subKode = trim($sub['kode']);
                            $rSum = isset($realAngMap['h_' . $hId]) ? $realAngMap['h_' . $hId] : (isset($realAngMap['c_' . $subKode]) ? $realAngMap['c_' . $subKode] : 0);
                            $kegRealAnggaran += $rSum;
                        }

                        $kCapaianAng = ($kegAnggaran > 0 && $kegRealAnggaran > 0) ? round(($kegRealAnggaran / $kegAnggaran) * 100, 2) : 0;

                        $progTotalAnggaran += $kegAnggaran;
                        $progTotalRealAnggaran += $kegRealAnggaran;

                        // Saved evaluasi Kegiatan
                        $savedKeg = isset($savedByTipeKode['kegiatan_' . $kKode]) ? $savedByTipeKode['kegiatan_' . $kKode] : 
                                    (isset($savedByUraian['kegiatan_' . strtolower(trim($kName))]) ? $savedByUraian['kegiatan_' . strtolower(trim($kName))] : null);
                        ...
                        $kegTinjut = '';
                        if ($savedKeg) {
                            $kegDbId = (int)$savedKeg['id'];
                            $matchedSavedIds[$kegDbId] = true;
                            $kegPermasalahan = $savedKeg['permasalahan'] ?? '';
                            $kegUpaya = $savedKeg['upaya'] ?? '';
                            $kegTinjut = $savedKeg['tinjut'] ?? '';
                        } else {
                            $virtualIdCounter++;
                            $kegDbId = $virtualIdCounter;
                        }
                        ...
                        $kegiatanItems[] = [
                            ...
                            'realisasi_anggaran' => $kegRealAnggaran,
                            'capaian_anggaran' => $kCapaianAng,
                            'permasalahan' => $kegPermasalahan,
                            'upaya' => $kegUpaya,
                            'tinjut' => $kegTinjut
                        ];
                    }

                    $pCapaianAng = ($progTotalAnggaran > 0 && $progTotalRealAnggaran > 0) ? round(($progTotalRealAnggaran / $progTotalAnggaran) * 100, 2) : 0;
                    ...
                    $programRow = [
                        ...
                        'capaian' => $pCapaianKin,
                        'anggaran' => $progTotalAnggaran,
                        'realisasi_anggaran' => $progTotalRealAnggaran,
                        'capaian_anggaran' => $pCapaianAng,
                        'permasalahan' => $progPermasalahan,
                        'upaya' => $progUpaya,
                        'tinjut' => $progTinjut
                    ];
        ...
        // 6. Jika ada data lkpj_capaian_program_kegiatan yang belum masuk (manual data), tambahkan ke groups
        foreach ($savedRows as $sr) {
            if (isset($matchedSavedIds[(int)$sr['id']])) continue;
            if ($sr['tipe'] === 'program') {
                $pId = (int)$sr['id'];
                $kegs = [];
                foreach ($savedRows as $subSr) {
                    if ($subSr['tipe'] === 'kegiatan' && (int)$subSr['parent_id'] === $pId) {
                        $matchedSavedIds[(int)$subSr['id']] = true;
                        $kegs[] = $subSr;
                    }
                }
                $groups[] = [
                    'id' => $pId,
                    'kebijakan' => $sr['kebijakan'],
                    'program' => $sr,
                    'kegiatan' => $kegs
                ];
                $matchedSavedIds[$pId] = true;
            }
        }
    ...
    public function GetCapaianProgramKegiatan() {
        ...
        $tahun = (int)($this->input->post('tahun', TRUE) ?: 2026);
        $urusan = $this->input->post('urusan', TRUE) ?: 'Semua Urusan';
        $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: 1);
    ...
    public function SaveCapaianProgramKegiatan() {
        ...
        try {
            ...
            $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: ($this->get_instansi_id() ?: 1));
            $kode_wilayah = $this->get_kode_wilayah() ?: '35.12';
            $urusan = trim($this->input->post('urusan', TRUE) ?: 'Semua Urusan');
            ...
            if ($id > 0 && $this->db->where('id', $id)->count_all_results('lkpj_capaian_program_kegiatan') > 0) {
                $this->db->where('id', $id)->update('lkpj_capaian_program_kegiatan', $saveData);
                $msg = 'Data Capaian Program/Kegiatan berhasil diperbarui.';
            } else {
                // Cari apakah sudah ada data untuk instansi, tahun, tipe, kode/uraian
                $checkQuery = $this->db->where('instansi_id', $instansi_id)
                    ->where('tahun', $tahun)
                    ->where('tipe', $tipe)
                    ->where('deleted_at IS NULL');
                if (!empty($kode)) {
                    $checkQuery->where('kode', $kode);
                } else {
                    $checkQuery->where('uraian', $uraian);
                }
                $existing = $checkQuery->get('lkpj_capaian_program_kegiatan')->row_array();
                if ($existing) {
                    $id = (int)$existing['id'];
                    $this->db->where('id', $id)->update('lkpj_capaian_program_kegiatan', $saveData);
                    $msg = 'Data Capaian Program/Kegiatan berhasil diperbarui.';
    ...
    public function SaveEvaluasiProgramKegiatan() {
        ...
        try {
            ...
            $indikator = trim($this->input->post('indikator', TRUE));
            $satuan = trim($this->input->post('satuan', TRUE));
            $target = (float)$this->input->post('target', TRUE);
            $realisasi = (float)$this->input->post('realisasi', TRUE);
            $capaian = (float)$this->input->post('capaian', TRUE);
            $anggaran = (float)$this->input->post('anggaran', TRUE);
            $realisasi_anggaran = (float)$this->input->post('realisasi_anggaran', TRUE);
            $capaian_anggaran = (float)$this->input->post('capaian_anggaran', TRUE);
            ...
            if ($id > 0 && $this->db->where('id', $id)->count_all_results('lkpj_capaian_program_kegiatan') > 0) {
                ...
                // Cek apakah ada record di lkpj_capaian_program_kegiatan berdasarkan kode / uraian
                $checkQuery = $this->db->where('instansi_id', $instansi_id)
                    ->where('tahun', $tahun)
                    ->where('tipe', $tipe)
                    ->where('deleted_at IS NULL');
                ...
                if ($existing) {
                    ...
                    $maxUrutan = $this->db->select_max('urutan')->where('tahun', $tahun)->where('instansi_id', $instansi_id)->get('lkpj_capaian_program_kegiatan')->row_array();
                    $insertData['urutan'] = isset($maxUrutan['urutan']) ? ((int)$maxUrutan['urutan'] + 1) : 1;
                    $this->db->insert('lkpj_capaian_program_kegiatan', $insertData);
                    $id = $this->db->insert_id();
                }
            }
    ...
    private function _ensure_tinjut_dprd_table() {
        if (!$this->db->table_exists('lkpj_rekomendasi_dprd_master')) {
            $this->db->query("CREATE TABLE IF NOT EXISTS `lkpj_rekomendasi_dprd_master` (
              `id` INT(11) NOT NULL AUTO_INCREMENT,
              `kodewilayah` VARCHAR(20) DEFAULT '35.12',
              `instansi_id` INT(11) DEFAULT 0,
              `tahun` INT(4) NOT NULL DEFAULT 2026,
              ...
              `deleted_at` DATETIME DEFAULT NULL,
              PRIMARY KEY (`id`),
              INDEX `idx_rekom_daerah` (`kodewilayah`, `tahun`, `instansi_id`, `deleted_at`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        } else {
            if (!$this->db->field_exists('instansi_id', 'lkpj_rekomendasi_dprd_master')) {
                $this->db->query("ALTER TABLE `lkpj_rekomendasi_dprd_master` ADD COLUMN `instansi_id` INT(11) DEFAULT 0 AFTER `kodewilayah`, ADD INDEX (`instansi_id`)");
            }
        }

        if (!$this->db->table_exists('lkpj_tinjut_rekomendasi_dprd')) {
            $this->db->query("CREATE TABLE IF NOT EXISTS `lkpj_tinjut_rekomendasi_dprd` (
              `id` INT(11) NOT NULL AUTO_INCREMENT,
              ...
              INDEX `idx_tr_rekom` (`rekomendasi_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        } else {
            if (!$this->db->field_exists('rekomendasi_id', 'lkpj_tinjut_rekomendasi_dprd')) {
                $this->db->query("ALTER TABLE `lkpj_tinjut_rekomendasi_dprd` ADD COLUMN `rekomendasi_id` INT(11) DEFAULT NULL AFTER `id`, ADD INDEX (`rekomendasi_id`)");
            }
        }

        // Seed master if master table is empty
        $countMaster = $this->db->where('deleted_at IS NULL')->count_all_results('lkpj_rekomendasi_dprd_master');
        if ($countMaster == 0) {
            $old_items = $this->db->where('deleted_at IS NULL')->group_by(['tahun', 'rekomendasi'])->get('lkpj_tinjut_rekomendasi_dprd')->result_array();
            if (!empty($old_items)) {
                foreach ($old_items as $old) {
                    if (!empty(trim($old['rekomendasi']))) {
                        $this->db->insert('lkpj_rekomendasi_dprd_master', [
                            'kodewilayah' => !empty($old['kodewilayah']) ? $old['kodewilayah'] : '35.12',
                            'instansi_id' => !empty($old['instansi_id']) ? (int)$old['instansi_id'] : 0,
                            'tahun' => !empty($old['tahun']) ? (int)$old['tahun'] : 2026,
                            'rekomendasi' => trim($old['rekomendasi']),
                            'created_at' => date('Y-m-d H:i:s'),
                            'updated_at' => date('Y-m-d H:i:s')
                        ]);
                        $m_id = $this->db->insert_id();
                        $this->db->where('rekomendasi', $old['rekomendasi'])->update('lkpj_tinjut_rekomendasi_dprd', ['rekomendasi_id' => $m_id]);
                    }
                }
            } else {
                $seeds = [
        ...
        // Synchronize instansi_id in tinjut with master when master has specific instansi_id
        $this->db->query("UPDATE `lkpj_tinjut_rekomendasi_dprd` t 
            INNER JOIN `lkpj_rekomendasi_dprd_master` m ON t.rekomendasi_id = m.id 
            SET t.instansi_id = m.instansi_id 
            WHERE t.instansi_id != m.instansi_id AND m.instansi_id > 0 AND m.deleted_at IS NULL");
    }

    private function _get_tinjut_rekomendasi_dprd_items($tahun, $instansi_id, $KodeWilayah = null, $is_role_4 = false) {
        $this->_ensure_tinjut_dprd_table();

        // Get map of instansi filtered by region if set
        $this->db->select('id, nama')->where('deleted_at IS NULL');
        if (!empty($KodeWilayah)) {
            $this->db->where('kodewilayah', $KodeWilayah);
        }
        $instList = $this->db->get('akun_instansi')->result_array();
        $instMap = [];
        foreach ($instList as $ins) {
            $instMap[(int)$ins['id']] = $ins['nama'];
        }

        $this->db->where('deleted_at IS NULL');
        if ($tahun > 0) $this->db->where('tahun', (int)$tahun);
        if (!empty($KodeWilayah)) {
            $this->db->where('kodewilayah', $KodeWilayah);
        }

        // If filtering for a specific instansi
        if ($instansi_id > 0) {
            $this->db->group_start()
                ->where('instansi_id', (int)$instansi_id)
                ->or_where('instansi_id', 0)
                ->or_where('instansi_id IS NULL')
                ->group_end();
        }

        $masters = $this->db->order_by('id', 'ASC')->get('lkpj_rekomendasi_dprd_master')->result_array();
        ...
        if ($instansi_id > 0) $this->db->where('instansi_id', (int)$instansi_id);
        $tinjuts = $this->db->get('lkpj_tinjut_rekomendasi_dprd')->result_array();

        $tinjut_by_rekom = [];
        $tinjut_by_text = [];
        foreach ($tinjuts as $t) {
            if (!empty($t['rekomendasi_id'])) {
                $tinjut_by_rekom[$t['rekomendasi_id']] = $t;
            }
            if (!empty($t['rekomendasi'])) {
                $tinjut_by_text[trim($t['rekomendasi'])] = $t;
            }
        }
        ...
        foreach ($masters as $m) {
            $matched = null;
            if (!empty($m['id']) && isset($tinjut_by_rekom[$m['id']])) {
                $matched = $tinjut_by_rekom[$m['id']];
            } elseif (!empty($m['rekomendasi']) && isset($tinjut_by_text[trim($m['rekomendasi'])])) {
                $matched = $tinjut_by_text[trim($m['rekomendasi'])];
            }

            if ($matched) {
                $usedTinjutIds[] = $matched['id'];
            }

            $tagId = !empty($m['instansi_id']) ? (int)$m['instansi_id'] : 0;
            $tagNama = ($tagId > 0 && isset($instMap[$tagId])) ? $instMap[$tagId] : 'Semua Perangkat Daerah';

            $items[] = [
                'id' => $matched ? (int)$matched['id'] : 0,
                'rekomendasi_id' => (int)$m['id'],
                'target_instansi_id' => $tagId,
                'target_instansi_nama' => $tagNama,
                'kodewilayah' => $m['kodewilayah'],
                'instansi_id' => (int)$instansi_id,
                'tahun' => (int)$m['tahun'],
                'rekomendasi' => $m['rekomendasi'],
                'tindak_lanjut' => $matched ? ($matched['tindak_lanjut'] ?: '') : '',
                'tujuan_masalah' => $matched ? ($matched['tujuan_masalah'] ?: '') : '',
                'is_master' => 1
            ];
        }

        // Standalone tinjut records
        foreach ($tinjuts as $t) {
            if (!in_array($t['id'], $usedTinjutIds)) {
                if (!empty($t['rekomendasi_id'])) {
                    $mRow = $this->db->where('id', (int)$t['rekomendasi_id'])->where('deleted_at IS NULL')->get('lkpj_rekomendasi_dprd_master')->row_array();
                    if ($mRow) {
                        if ($instansi_id > 0 && !empty($mRow['instansi_id']) && (int)$mRow['instansi_id'] !== (int)$instansi_id) {
                            continue;
                        }
                    }
                }
                if (!empty($KodeWilayah) && !empty($t['kodewilayah']) && $t['kodewilayah'] !== $KodeWilayah) {
                    continue;
                }
                $tagId = !empty($t['instansi_id']) ? (int)$t['instansi_id'] : 0;
                $tagNama = ($tagId > 0 && isset($instMap[$tagId])) ? $instMap[$tagId] : 'Semua Perangkat Daerah';

                $items[] = [
                    'id' => (int)$t['id'],
                    'rekomendasi_id' => !empty($t['rekomendasi_id']) ? (int)$t['rekomendasi_id'] : 0,
                    'target_instansi_id' => $tagId,
                    'target_instansi_nama' => $tagNama,
                    'kodewilayah' => $t['kodewilayah'],
                    'instansi_id' => (int)$t['instansi_id'],
                    'tahun' => (int)$t['tahun'],
                    'rekomendasi' => $t['rekomendasi'] ?: '',
                    'tindak_lanjut' => $t['tindak_lanjut'] ?: '',
                    'tujuan_masalah' => $t['tujuan_masalah'] ?: '',
                    'is_master' => 0
                ];
            }
        }
    ...
    public function PengisianTinjutRekomendasiDPRDn1() {
        ...
        $this->db->select('id, nama')->from('akun_instansi')->where('deleted_at IS NULL');
        if (!empty($KodeWilayah)) {
            $this->db->where('kodewilayah', $KodeWilayah);
        }
        $Data['ListInstansi'] = $this->db->order_by('nama', 'ASC')->get()->result_array();
        $Data['ListTahun'] = [2027, 2026, 2025, 2024, 2023];
        $Data['TahunAktif'] = (int)$tahun;
        $activeInstansi = $filter_instansi !== null && $filter_instansi !== '' ? (int)$filter_instansi : ($is_role_4 ? (int)$instansi_id : 0);
        $Data['FilterInstansi'] = $activeInstansi;
        $Data['filterInstansi'] = $activeInstansi;
        $Data['ActiveInstansiId'] = $activeInstansi;
        ...
        if (!$is_logged_in && empty($KodeWilayah)) {
            ...
            $items = $this->_get_tinjut_rekomendasi_dprd_items($tahun, $Data['FilterInstansi'], $KodeWilayah, $is_role_4);
        }
    ...
    public function GetTinjutRekomendasiDPRD() {
        ...
        $post_instansi = $this->input->post('instansi_id', TRUE);
        if ($post_instansi !== null && $post_instansi !== '') {
            $instansi_id = (int)$post_instansi;
        } else {
            $instansi_id = $is_role_4 ? (int)($this->get_instansi_id() ?: 0) : 0;
        }

        $items = $this->_get_tinjut_rekomendasi_dprd_items($tahun, $instansi_id, $KodeWilayah, $is_role_4);
    ...
    public function SaveTinjutRekomendasiDPRD() {
        ...
        try {
            ...
            $id = (int)$this->input->post('id', TRUE);
            $rekomendasi_id = (int)$this->input->post('rekomendasi_id', TRUE);
            $target_instansi_id = (int)$this->input->post('target_instansi_id', TRUE);
            $tahun = (int)($this->input->post('tahun', TRUE) ?: 2026);
            $instansi_id = (int)($this->input->post('instansi_id', TRUE) ?: ($this->get_instansi_id() ?: 1));
            $kode_wilayah = $this->get_kode_wilayah() ?: '35.12';
            ...
            if ($action_type === 'daerah_master') {
                if (empty($rekomendasi)) {
                    throw new Exception('Uraian Rekomendasi DPRD wajib diisi.');
                }

                // If rekomendasi_id is 0 but $id > 0, check if that tinjut row has a master
                if ($rekomendasi_id <= 0 && $id > 0) {
                    $tRow = $this->db->where('id', $id)->get('lkpj_tinjut_rekomendasi_dprd')->row_array();
                    if ($tRow && !empty($tRow['rekomendasi_id'])) {
                        $rekomendasi_id = (int)$tRow['rekomendasi_id'];
                    }
                }

                if ($rekomendasi_id > 0) {
                    $oldMaster = $this->db->where('id', $rekomendasi_id)->get('lkpj_rekomendasi_dprd_master')->row_array();
                    $targetChanged = ($oldMaster && (int)$oldMaster['instansi_id'] !== (int)$target_instansi_id);

                    $this->db->where('id', $rekomendasi_id)->update('lkpj_rekomendasi_dprd_master', [
                        'rekomendasi' => $rekomendasi,
                        'instansi_id' => $target_instansi_id,
                        'tahun' => $tahun,
                        'kodewilayah' => $kode_wilayah,
                        'updated_at' => date('Y-m-d H:i:s')
                    ]);
                    ...
                    if ($targetChanged) {
                        $tinjutUpdate['tindak_lanjut'] = null;
                        $tinjutUpdate['tujuan_masalah'] = null;
                    }

                    // Sync all tinjut records for this master to the new target_instansi_id
                    $this->db->where('rekomendasi_id', $rekomendasi_id)->update('lkpj_tinjut_rekomendasi_dprd', $tinjutUpdate);
                    ...
                    if ($id > 0) {
                        ...
                        if ($targetChanged) {
                            $singleUpdate['tindak_lanjut'] = null;
                            $singleUpdate['tujuan_masalah'] = null;
                        }
                        $this->db->where('id', $id)->update('lkpj_tinjut_rekomendasi_dprd', $singleUpdate);
                    }

                    $msg = 'Rekomendasi DPRD Daerah berhasil diperbarui.';
                } else {
                    $existingMaster = $this->db->where('tahun', $tahun)
                        ->where('kodewilayah', $kode_wilayah)
                        ->where('rekomendasi', $rekomendasi)
                        ->where('deleted_at IS NULL')
                        ->get('lkpj_rekomendasi_dprd_master')
                        ->row_array();

                    if ($existingMaster) {
                        $rekomendasi_id = (int)$existingMaster['id'];
                        $targetChanged = ((int)$existingMaster['instansi_id'] !== (int)$target_instansi_id);
                        ...
                        $rekomendasi_id = $this->db->insert_id();
                    }
                    ...
                    $msg = 'Rekomendasi DPRD Daerah berhasil disimpan.';
                }
            } else {
                // Instansi input Tindak Lanjut & Tujuan / Masalah
                if (empty($tindak_lanjut)) {
                    throw new Exception('Tindak Lanjut yang dilakukan wajib diisi.');
                }
                if (empty($tujuan_masalah)) {
                    throw new Exception('Tujuan / Masalah yang diselesaikan wajib diisi.');
                }

                $existing = null;
                if ($id > 0) {
                    $existing = $this->db->where('id', $id)->where('deleted_at IS NULL')->get('lkpj_tinjut_rekomendasi_dprd')->row_array();
                }
                if (!$existing && $rekomendasi_id > 0) {
                    $existing = $this->db->where('rekomendasi_id', $rekomendasi_id)
                        ->where('instansi_id', $instansi_id)
                        ->where('tahun', $tahun)
                        ->where('deleted_at IS NULL')
                        ->get('lkpj_tinjut_rekomendasi_dprd')->row_array();
                }

                if (empty($rekomendasi) && $rekomendasi_id > 0) {
                    $masterRow = $this->db->where('id', $rekomendasi_id)->get('lkpj_rekomendasi_dprd_master')->row_array();
                    if ($masterRow) {
                        $rekomendasi = $masterRow['rekomendasi'];
                    }
                }

                if ($existing) {
                    $saveData = [
                        'rekomendasi_id' => $rekomendasi_id ?: $existing['rekomendasi_id'],
                        'tindak_lanjut' => $tindak_lanjut,
                        'tujuan_masalah' => $tujuan_masalah,
                        'updated_at' => date('Y-m-d H:i:s')
                    ];
                    if (!empty($rekomendasi)) {
                        $saveData['rekomendasi'] = $rekomendasi;
                    }
                    $this->db->where('id', $existing['id'])->update('lkpj_tinjut_rekomendasi_dprd', $saveData);
                    $id = $existing['id'];
                    $msg = 'Tindak Lanjut Rekomendasi DPRD berhasil disimpan.';
                } else {
                    $saveData = [
                        'rekomendasi_id' => $rekomendasi_id,
                        'kodewilayah' => $kode_wilayah,
    ...
    public function DeleteTinjutRekomendasiDPRD() {
        ...
        try {
            $id = (int)$this->input->post('id', TRUE);
            $rekomendasi_id = (int)$this->input->post('rekomendasi_id', TRUE);
            $is_master = (int)$this->input->post('is_master', TRUE);

            if ($is_master && $rekomendasi_id > 0) {
                $this->db->where('id', $rekomendasi_id)->update('lkpj_rekomendasi_dprd_master', [
                    'deleted_at' => date('Y-m-d H:i:s')
                ]);
                $this->db->where('rekomendasi_id', $rekomendasi_id)->update('lkpj_tinjut_rekomendasi_dprd', [
                    'deleted_at' => date('Y-m-d H:i:s')
                ]);
                $msg = 'Rekomendasi DPRD Daerah berhasil dihapus.';
            } elseif ($id > 0) {
                $this->db->where('id', $id)->update('lkpj_tinjut_rekomendasi_dprd', [
                    'tindak_lanjut' => null,
                    'tujuan_masalah' => null,
                ...
                $msg = 'Tindak Lanjut instansi berhasil direset.';
            } else {
