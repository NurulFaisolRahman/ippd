...
<script>
    ...
    $("#PeriodeIsuKLHSNasional").change(function(){
        if ($("#PeriodeIsuKLHSNasional").val() == "") {
            ...
            $.post(BaseURL+"Admin/GetKementerianIsu", {TahunMulai : $("#PeriodeIsuKLHSNasional").val()}).done(function(Respon) {
    ...
    $("#Kementerian").change(function(){
        if ($("#Kementerian").val() == "") {
            ...
            $.post(BaseURL+"Admin/GetIsuKLHSNasional", {Id : $("#Kementerian").val()}).done(function(Respon) {
                var Data = JSON.parse(Respon)
                var Isu = ''
                for (let i = 0; i < Data.length; i++) {
    ...
    $("#EditPeriodeIsuKLHSNasional").change(function(){
        if ($(this).val() == "") {
            ...
            $.post(BaseURL+"Admin/GetKementerianIsu", {TahunMulai: $(this).val()}).done(function(Respon) {
                var Data = JSON.parse(Respon);
                var Kementerian = '<option value="">-- Pilih Kementerian --</option>';
                for (let i = 0; i < Data.length; i++) {
                    Kementerian += '<option value="'+Data[i].Id+'">'+Data[i].NamaKementerian+'</option>';
    ...
    $("#EditKementerian").change(function(){
        if ($(this).val() == "") {
            ...
            $.post(BaseURL+"Admin/GetIsuKLHSNasional", {Id: $(this).val()}).done(function(Respon) {
                var Data = JSON.parse(Respon);
                var Isu = '';
                for (let i = 0; i < Data.length; i++) {
    ...
    $(document).on("click", ".Edit", function() {
        ...
        $.post(BaseURL + "Admin/GetPeriodeIsuKLHSNasional", {Id: kementerian}).done(function(Respon) {
