...
<?php
  ...
  $userLevel         = $_SESSION['Level'] ?? ($this->session->userdata('Level') ?? ($_SESSION['userLevel'] ?? $this->session->userdata('userLevel')));
  $userIdKementerian = $_SESSION['IdKementerian'] ?? $this->session->userdata('IdKementerian');
  $sessionUsername   = $_SESSION['Username'] ?? $this->session->userdata('Username');

  if (isset($userLevel) && (int)$userLevel === 1) {
    if (!empty($userIdKementerian)) {
      $rowKemen = $this->db->select('NamaKementerian')
        ->from('kementerian')
        ->where('Id', $userIdKementerian)
        ->where('deleted_at IS NULL', null, false)
        ->limit(1)
        ->get()
        ->row_array();

      $LoginInfo = $rowKemen['NamaKementerian'] ?? ($_SESSION['NamaKementerian'] ?? ($sessionUsername ?? 'Kementerian'));
    } elseif (!empty($sessionUsername)) {
      $rowAkun = $this->db->select('IdKementerian')
        ->from('akun')
      ...
      if (!empty($rowAkun['IdKementerian'])) {
        $rowKemen = $this->db->select('NamaKementerian')
          ->from('kementerian')
          ->where('Id', $rowAkun['IdKementerian'])
          ->where('deleted_at IS NULL', null, false)
          ->limit(1)
          ->get()
          ->row_array();
        $LoginInfo = $rowKemen['NamaKementerian'] ?? ($sessionUsername ?? 'Kementerian');
      } else {
        $LoginInfo = $_SESSION['NamaKementerian'] ?? $sessionUsername;
      }
    } else {
      $LoginInfo = $_SESSION['NamaKementerian'] ?? 'Kementerian';
    }
  } elseif (isset($userLevel) && (int)$userLevel === 0) {
    $LoginInfo = !empty($sessionUsername) ? $sessionUsername : 'Administrator';
  } elseif (!empty($userIdKementerian)) {
    $rowKemen = $this->db->select('NamaKementerian')
      ->from('kementerian')
      ->where('Id', $userIdKementerian)
      ->where('deleted_at IS NULL', null, false)
      ...
      ->get()
      ->row_array();
    $LoginInfo = $rowKemen['NamaKementerian'] ?? ($_SESSION['NamaKementerian'] ?? ($sessionUsername ?? ''));
  } elseif (!empty($_SESSION['NamaKementerian'])) {
    $LoginInfo = $_SESSION['NamaKementerian'];
  } elseif (!empty($sessionUsername)) {
    $LoginInfo = $sessionUsername;
  }
?>
...
<nav class="navbar">
  <div class="navbar-container">
    ...
    <div class="navbar-center">
      <a href="<?= base_url('Kementerian/Kementerian'); ?>" class="navbar-brand">
    ...
    <div class="navbar-menu">
      ...
      <?php if (!empty($_SESSION['isLoggedIn']) || (isset($userLevel) && in_array((int)$userLevel, [0, 1]))) { ?>
