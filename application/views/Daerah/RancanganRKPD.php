  ...
  <div class="renja-wrapper">
    ...
    <div class="topbar">
      <div class="topbar-row">
        <div class="app-title">
          <h1>Rancangan Rencana Kerja Pemerintah Daerah (Rancangan RKPD)</h1>
          <span>Data Program, Kegiatan, dan Sub Kegiatan bersumber dari Rancangan Renja Perangkat Daerah &mdash; menampilkan Rancangan RKPD Tahun <b id="activeYearLabel"><?= html_escape($TahunAktif) ?></b></span>
        </div>
    ...
    <div class="pagu-summary">
      ...
      <div class="pagu-item">
        <span class="pagu-label">Total Anggaran Rancangan RKPD</span>
        ...
        <table class="renja-table" id="renjaTable">
          <thead>
            <tr>
              ...
              <th colspan="2">Rancangan Renja</th>
          ...
          <tbody id="renjaBody">
            <tr>
              <td colspan="15" class="text-center" style="padding:40px; color:var(--slate-400);">Memuat data Rancangan RKPD...</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<div class="modal-backdrop-custom" id="modalKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Kegiatan (Rancangan RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveKegiatan"><i class="fa fa-save"></i> Simpan Rancangan RKPD</button>
    </div>
  </div>
</div>
...
<div class="modal-backdrop-custom" id="modalSubKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Sub Kegiatan (Rancangan RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveSubKegiatan"><i class="fa fa-save"></i> Simpan Rancangan RKPD</button>
    </div>
  </div>
</div>
...
function saveKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanRancanganRKPDKegiatan', {
    method: 'POST',
    body: fd,
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Kegiatan Rancangan RKPD berhasil disimpan.');
...
function saveSubKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanRancanganRKPDSubKegiatan', {
    method: 'POST',
    body: fd,
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Sub Kegiatan Rancangan RKPD berhasil disimpan.');
...
function loadRenjaDataServer(tahun, instansiId){
  ...
  let url = BASE_URL + 'Daerah/getRancanganRKPDJson?tahun=' + encodeURIComponent(y);
  ...
  .then(function(data){
    ...
    if(data.status === 'success' && data.data){
      ...
      showToast('Gagal memuat data Rancangan RKPD.', 'error');
    }
  })
  .catch(function(err){
    showLoading(false);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(btnSync){
    btnSync.addEventListener('click', function(){
      ...
      if(!confirm('Apakah Anda yakin ingin menyinkronkan ulang data Rancangan RKPD Tahun ' + y + ' dengan data Rancangan Renja?\n\nPerubahan kustom pada Rancangan RKPD tahun ' + y + ' akan direset dan dicocokkan kembali ke data Rancangan Renja.')){
        return;
      }
      showLoading(true);
      const fd = new FormData();
      ...
      fetch(BASE_URL + 'Daerah/resetRancanganRKPDDataFromRenja', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
      })
  ...
  <?php if (!$IsLoggedIn || !isset($_SESSION['KodeWilayah'])) { ?>
    ...
    if (btnFilter) {
      btnFilter.addEventListener('click', function(){
        ...
        .then(function(res){
          if (res.trim() === '1') {
            let redirectUrl = BASE_URL + 'Daerah/RancanganRKPD';
            if (instansiVal) {
  ...
  <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
    ...
    if (btnFilterInstansi) {
      btnFilterInstansi.addEventListener('click', function(){
        ...
        let redirectUrl = BASE_URL + 'Daerah/RancanganRKPD?tahun=' + encodeURIComponent(tahun);
