...
<div class="main-content">
    <div class="pk-card">
        ...
        <!-- FILTER WILAYAH SECTION (HANYA MUNCUL JIKA BELUM LOGIN SEBAGAI DAERAH) -->
        ...
        <div class="pk-header">
            <div class="pk-header-title">
                ...
                <div>
                    <h4>Pohon Kinerja Daerah <?= !empty($NamaWilayah) ? '- ' . html_escape($NamaWilayah) : '' ?></h4>
            ...
            <div class="pk-ultimate-select-box">
                <label for="select-ultimate-outcome"><i class="fa fa-crown"></i> Pilih Ultimate:</label>
                <select id="select-ultimate-outcome" class="pk-select-ultimate">
...
(function () {
    ...
    function updateLegendCounts(counts) {
        document.querySelectorAll('.pk-legend .legend-item').forEach(item => {
            ...
            else if (text.includes('sektor')) key = 'level2';
            else if (text.includes('taktikal')) key = 'level3';
            else if (text.includes('immediate')) key = 'level4';
            else if (text.includes('output')) key = 'level5';
    ...
    const selectUltimateEl = document.getElementById('select-ultimate-outcome');

    // Cek jika data kosong dari backend
    if (!chartData || !chartData.children || chartData.children.length === 0) {
        document.getElementById('chart-svg').style.display = 'none';
        if (selectUltimateEl) {
            selectUltimateEl.innerHTML = '<option value="">-- Tidak Ada Data --</option>';
            selectUltimateEl.disabled = true;
        }
        ...
        if (totalData.level1 > 0 || totalData.level2 > 0 || totalData.level3 > 0 || totalData.level4 > 0 || totalData.level5 > 0) {
            emptyHtml += `
                <div class="stats-card" style="margin-top:16px;">
                    ...
                    <div class="stat-item"><div class="stat-dot" style="background:var(--l2)"></div><span class="stat-value">${totalData.level2 || 0}</span><span class="stat-label">Int. Sektor</span></div>
                    <div class="stat-item"><div class="stat-dot" style="background:var(--l3)"></div><span class="stat-value">${totalData.level3 || 0}</span><span class="stat-label">Int. Taktikal</span></div>
                    <div class="stat-item"><div class="stat-dot" style="background:var(--l4)"></div><span class="stat-value">${totalData.level4 || 0}</span><span class="stat-label">Immediate</span></div>
                    <div class="stat-item"><div class="stat-dot" style="background:var(--l5)"></div><span class="stat-value">${totalData.level5 || 0}</span><span class="stat-label">Output</span></div>
                </div>
            `;
        } else {
    ...
    // Populate dropdown Ultimate Outcome
    if (selectUltimateEl) {
        selectUltimateEl.innerHTML = '<option value="">-- Pilih Ultimate Outcome --</option>' +
        ...
        chartData.children.forEach((child, index) => {
            ...
            selectUltimateEl.appendChild(opt);
        });
    }

    // Node dimensions for D3 Tree
    ...
    function renderTreeForData(activeData) {
        ...
        const existingPrompt = document.getElementById('empty-select-prompt');
        ...
        if (!activeData || !activeData.children || activeData.children.length === 0) {
            ...
            const promptHtml = `
                <div class="pk-empty" id="empty-select-prompt">
                    ...
                    <p style="color: #64748b; font-size: 14px; max-width: 460px; text-align: center; line-height: 1.5; margin: 0;">
                        Silakan pilih <strong>Ultimate Outcome (Level 1)</strong> pada dropdown di bagian atas untuk menampilkan bagan pohon kinerja beserta seluruh rantai turunannya.
                    </p>
                </div>
            `;
            container.insertAdjacentHTML('beforeend', promptHtml);
            updateLegendCounts({ level1: 0, level2: 0, level3: 0, level4: 0, level5: 0 });
            return;
        }
    ...
    if (selectUltimateEl) {
        selectUltimateEl.addEventListener('change', function() {
