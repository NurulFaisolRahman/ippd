  ...
  <div class="renja-wrapper">
    ...
    <div class="topbar">
      <div class="topbar-row">
        <div class="app-title">
          <h1>Perubahan Rencana Kerja Pemerintah Daerah (Perubahan RKPD)</h1>
          <span>Data Program, Kegiatan, dan Sub Kegiatan bersumber dari Perubahan Renja Perangkat Daerah &mdash; menampilkan Perubahan RKPD Tahun <b id="activeYearLabel"><?= html_escape($TahunAktif) ?></b></span>
        </div>
    ...
    <div class="pagu-summary">
      ...
      <div class="pagu-item">
        <span class="pagu-label">Total Anggaran Perubahan RKPD</span>
    ...
    <div class="table-card">
      ...
      <div class="table-container">
        <table class="renja-table" id="renjaTable">
          ...
          <tbody id="renjaBody">
            <tr>
              <td colspan="15" class="text-center" style="padding:40px; color:var(--slate-400);">Memuat data Perubahan RKPD...</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<div class="modal-backdrop-custom" id="modalKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Kegiatan (Perubahan RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveKegiatan"><i class="fa fa-save"></i> Simpan Perubahan RKPD</button>
    </div>
  </div>
</div>
...
<div class="modal-backdrop-custom" id="modalSubKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Sub Kegiatan (Perubahan RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveSubKegiatan"><i class="fa fa-save"></i> Simpan Perubahan RKPD</button>
    </div>
  </div>
</div>
...
function saveKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanPerubahanRKPDKegiatan', {
    method: 'POST',
    body: fd,
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Kegiatan Perubahan RKPD berhasil disimpan.');
...
function saveSubKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanPerubahanRKPDSubKegiatan', {
    method: 'POST',
    body: fd,
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Sub Kegiatan Perubahan RKPD berhasil disimpan.');
...
function loadRenjaDataServer(tahun, instansiId){
  ...
  let url = BASE_URL + 'Daerah/getPerubahanRKPDJson?tahun=' + encodeURIComponent(y);
  ...
  .then(function(data){
    ...
    if(data.status === 'success' && data.data){
      ...
      showToast('Gagal memuat data Perubahan RKPD.', 'error');
    }
  })
  .catch(function(err){
    showLoading(false);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(btnSync){
    btnSync.addEventListener('click', function(){
      ...
      if(!confirm('Apakah Anda yakin ingin menyinkronkan ulang data Perubahan RKPD Tahun ' + y + ' dengan data Perubahan Renja?\n\nPerubahan kustom pada Perubahan RKPD tahun ' + y + ' akan direset dan dicocokkan kembali ke data Perubahan Renja.')){
        return;
      }
      showLoading(true);
      const fd = new FormData();
      ...
      fetch(BASE_URL + 'Daerah/resetPerubahanRKPDDataFromRenja', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
      })
  ...
  <?php if (!$IsLoggedIn || !isset($_SESSION['KodeWilayah'])) { ?>
    ...
    if (btnFilter) {
      btnFilter.addEventListener('click', function(){
        ...
        .then(function(res){
          if (res.trim() === '1') {
            let redirectUrl = BASE_URL + 'Daerah/PerubahanRKPD';
            if (instansiVal) {
  ...
  <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
    ...
    if (btnFilterInstansi) {
      btnFilterInstansi.addEventListener('click', function(){
        ...
        let redirectUrl = BASE_URL + 'Daerah/PerubahanRKPD?tahun=' + encodeURIComponent(tahun);
