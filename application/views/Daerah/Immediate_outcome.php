    ...
    <div class="container-fluid">
      ...
      <div style="background: white; padding: 12px 20px; margin-bottom: 25px; border: 1px solid #dee2e6; border-radius: 5px;">
        <div style="display: flex; gap: 25px; flex-wrap: wrap;">
          ...
          <a href="<?=base_url('Daerah/Immediate_outcome')?>" class="nav-link active">Immediate outcome</a>
                ...
                <tbody>
                  ...
                  ?>
                    <tr data-id="<?= (int)$row['id'] ?>">
                      ...
                      <td class="align-middle"><?= nl2br(html_escape($row['taktikal_kinerja'] ?? '—')) ?></td>
                      ...
                      <?php if (isset($_SESSION['Level']) && $_SESSION['Level'] == 3) { ?>
                        <td class="text-center align-middle">
                          <button class="btn btn-sm btn-primary btn-edit-level4"
                                data-id="<?= $row['id'] ?>"
                                data-taktikal="<?= htmlspecialchars($row['intermediate_taktikal_id'] ?? '', ENT_QUOTES) ?>"
                          ...
                          <button class="btn btn-sm btn-danger btn-hapus-level4" data-id="<?= $row['id'] ?>">
                  ...
                  <?php } } else { ?>
                    <tr>
                      <td colspan="<?= (isset($_SESSION['Level']) && $_SESSION['Level'] == 3) ? 4 : 3 ?>" class="text-center py-5 text-muted">
                        ...
                        Belum ada data Immediate Outcome<br>
                        <?php if (isset($_SESSION['Level']) && $_SESSION['Level'] == 3) { ?>
...
<!-- MODAL IMMEDIATE OUTCOME (LEVEL 4) -->
  ...
  <div class="modal-dialog modal-xl" style="max-width: 1400px; width: 95%; margin: 30px auto;">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button class="btn btn-danger" id="btn-hapus-modal-level4" style="display:none;">Delete</button>
        <button class="btn btn-primary" id="btn-simpan-level4">
          Simpan Perubahan
        </button>
      </div>
...
$(document).ready(function() {
  ...
  function addCrosscutting(pd='', pohon='', info='') {
    $("#crosscutting-body-level4").append(`
      <tr>
  ...
  // ================= LOAD DROPDOWN DINAS UNTUK LEVEL 4 =================
  function loadDinasOptionsLevel4(selectedDinasId = '') {
    $.ajax({
      url: BaseURL + 'Daerah/get_daftar_dinas_immediate',
      type: 'GET',
      dataType: 'json',
      beforeSend: function() {
        $('#dinas_filter_l4').html('<option value="">Loading...</option>');
      },
      success: function(data) {
        ...
        $('#dinas_filter_l4').html(options);
        
        // Inisialisasi Select2 untuk Dinas
        if ($('#dinas_filter_l4').hasClass('select2-hidden-accessible')) {
          $('#dinas_filter_l4').select2('destroy');
        }
        $('#dinas_filter_l4').select2({
        ...
        if (selectedDinasId) {
          $('#pelaksana_group_l4').show();
        } else {
          $('#pelaksana_group_l4').hide();
        }
      },
      error: function() {
        $('#dinas_filter_l4').html('<option value="">Gagal memuat data</option>');
      }
    });
  }

  // ================= LOAD PELAKSANA BERDASARKAN DINAS UNTUK LEVEL 4 =================
  function loadPelaksanaByDinasLevel4(dinasId, selectedPelaksanaId = '') {
    if (!dinasId) {
      $('#pelaksana_group_l4').hide();
      $('#pelaksana_l4').html('<option value="">-- Pilih Pelaksana --</option>');
      return;
    }
    ...
    $.ajax({
      url: BaseURL + 'Daerah/get_pelaksana_immediate_by_dinas',
      type: 'POST',
      data: { dinas_id: dinasId },
      dataType: 'json',
      beforeSend: function() {
        $('#pelaksana_l4').html('<option value="">Loading...</option>');
      },
      success: function(data) {
        ...
        $('#pelaksana_l4').html(options);
        
        // Re-inisialisasi Select2 untuk Pelaksana
        if ($('#pelaksana_l4').hasClass('select2-hidden-accessible')) {
          $('#pelaksana_l4').select2('destroy');
        }
        $('#pelaksana_l4').select2({
        ...
        if (selectedPelaksanaId) {
          $('#pelaksana_l4').val(selectedPelaksanaId).trigger('change');
        }
      },
      error: function() {
        $('#pelaksana_l4').html('<option value="">Gagal memuat data</option>');
      }
    });
  }

  // ================= SAAT DINAS BERUBAH =================
  $('#dinas_filter_l4').on('change', function() {
    let dinasId = $(this).val();
    loadPelaksanaByDinasLevel4(dinasId, '');
  });

  // ================= BUTTON TAMBAH =================
  $("#btn-add-indikator-l4").click(() => addIndikator("#indikator-container-level4"));
  $("#btn-add-inovasi-l4").click(() => addInovasi("#inovasi-container-level4"));
  $("#btn-add-outcome-inovasi-l4").click(() => addOutcomeInovasi("#outcome-inovasi-container-level4"));
  $("#btn-add-output-inovasi-l4").click(() => addOutputInovasi("#output-inovasi-container-level4"));
  $("#btn-add-crosscutting-l4").click(() => addCrosscutting());

  // ================= RESET MODAL SAAT TAMBAH BARU =================
  $('#modalLevel4').on('show.bs.modal', function(e){
    if (!e.relatedTarget) return;

    if (!$(e.relatedTarget).hasClass('btn-edit-level4')) {
      $('#id_level4').val('');
      $('#taktikal_id').val('');
      $('#kinerja_level4').val('');
      ...
      addIndikator('#indikator-container-level4');
    }
  });
  ...
  $(document).on('click', '.btn-edit-level4', function(){
    ...
    if (pelaksanaId) {
      $.ajax({
        url: BaseURL + 'Daerah/get_pelaksana_immediate_detail',
        type: 'POST',
        data: { id: pelaksanaId },
        dataType: 'json',
        success: function(data) {
          if (data && data.dinas_id) {
            ...
            if (dinasIds.length > 0 && dinasIds[0]) {
              ...
              $('#dinas_filter_l4').val(dinasIds[0]).trigger('change');
              // Load pelaksana berdasarkan dinas dan set selected
              setTimeout(function() {
                loadPelaksanaByDinasLevel4(dinasIds[0], pelaksanaId);
              }, 500);
            } else {
    ...
    if (indikator) {
      indikator.split('|||').forEach(v=>{
        if(v.trim()) addIndikator('#indikator-container-level4', v.trim());
      });
    }
    if(!$('#indikator-container-level4 .field-row').length){
      addIndikator('#indikator-container-level4');
    }
    ...
    if (inovasi) {
      inovasi.split('|||').forEach(v=>{
        if(v.trim()) addInovasi('#inovasi-container-level4', v.trim());
      });
    }
    ...
    if (outcome) {
      outcome.split('|||').forEach(v=>{
        if(v.trim()) addOutcomeInovasi('#outcome-inovasi-container-level4', v.trim());
      });
    }
    ...
    if (output) {
      output.split('|||').forEach(v=>{
        if(v.trim()) addOutputInovasi('#output-inovasi-container-level4', v.trim());
      });
    }
  ...
  $('#btn-simpan-level4').click(function() {
    ...
    let taktikal_id = $('#taktikal_id').val();
    let kinerja = $('#kinerja_level4').val().trim();
    let pelaksanaId = $('#pelaksana_l4').val();
    ...
    // Kumpulkan indikator
    let indikator = [];
    $('#indikator-container-level4 input').each(function() {
      let v = $(this).val().trim();
    ...
    // Kumpulkan inovasi
    let inovasi = [];
    $('#inovasi-container-level4 input').each(function() {
      let v = $(this).val().trim();
      if (v) inovasi.push(v);
    });
    ...
    let outcome = [];
    $('#outcome-inovasi-container-level4 input').each(function() {
      let v = $(this).val().trim();
      if (v) outcome.push(v);
    });
    ...
    let output = [];
    $('#output-inovasi-container-level4 input').each(function() {
      let v = $(this).val().trim();
      if (v) output.push(v);
    });
    ...
    $.post(BaseURL + 'Daerah/Immediate_outcome_simpan', {
      ...
      if (res.status === 'success') {
        ...
        $('#btn-simpan-level4').prop('disabled', false).text('Simpan Perubahan');
      }
  ...
  $(document).on('click', '.btn-hapus-level4', function() {
    ...
    $.post(BaseURL + 'Daerah/Immediate_outcome_hapus', {id: id}, function(res) {
      if (res.status === 'success') {
        alert(res.message);
        location.reload();
      } else {
  ...
  // Load initial data untuk dinas jika modal dibuka (opsional)
  // Tidak perlu load di sini karena akan di-load saat modal dibuka
});
</script>