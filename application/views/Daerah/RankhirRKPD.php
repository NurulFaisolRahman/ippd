  ...
  <div class="renja-wrapper">
    ...
    <div class="topbar">
      <div class="topbar-row">
        <div class="app-title">
          <h1>Rancangan Akhir Rencana Kerja Pemerintah Daerah (Rankhir RKPD)</h1>
          <span>Data Program, Kegiatan, dan Sub Kegiatan bersumber dari Rankhir Renja Perangkat Daerah &mdash; menampilkan Rankhir RKPD Tahun <b id="activeYearLabel"><?= html_escape($TahunAktif) ?></b></span>
        </div>
    ...
    <div class="pagu-summary">
      ...
      <div class="pagu-item">
        <span class="pagu-label">Total Anggaran Rankhir RKPD</span>
    ...
    <div class="table-card">
      ...
      <div class="table-container">
        <table class="renja-table" id="renjaTable">
          ...
          <tbody id="renjaBody">
            <tr>
              <td colspan="15" class="text-center" style="padding:40px; color:var(--slate-400);">Memuat data Rankhir RKPD...</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<div class="modal-backdrop-custom" id="modalKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Kegiatan (Rankhir RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveKegiatan"><i class="fa fa-save"></i> Simpan Rankhir RKPD</button>
    </div>
  </div>
</div>
...
<div class="modal-backdrop-custom" id="modalSubKegiatan">
  <div class="modal-dialog-custom">
    <div class="modal-header-custom">
      <h3><i class="fa fa-pencil-square-o" style="color:var(--brand-green);"></i> Edit Sub Kegiatan (Rankhir RKPD)</h3>
    ...
    <div class="modal-footer-custom">
      ...
      <button type="button" class="btn-primary-custom" id="btnSaveSubKegiatan"><i class="fa fa-save"></i> Simpan Rankhir RKPD</button>
    </div>
  </div>
</div>
...
function saveKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanRankhirRKPDKegiatan', {
    method: 'POST',
    body: fd,
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Kegiatan Rankhir RKPD berhasil disimpan.');
...
function saveSubKegiatan(){
  ...
  fetch(BASE_URL + 'Daerah/simpanRankhirRKPDSubKegiatan', {
    method: 'POST',
    body: fd,
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      showToast(data.message || 'Sub Kegiatan Rankhir RKPD berhasil disimpan.');
...
function loadRenjaDataServer(tahun, instansiId){
  ...
  let url = BASE_URL + 'Daerah/getRankhirRKPDJson?tahun=' + encodeURIComponent(y);
  ...
  .then(function(data){
    ...
    if(data.status === 'success' && data.data){
      ...
      showToast('Gagal memuat data Rankhir RKPD.', 'error');
    }
  })
  .catch(function(err){
    showLoading(false);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(btnSync){
    btnSync.addEventListener('click', function(){
      ...
      if(!confirm('Apakah Anda yakin ingin menyinkronkan ulang data Rankhir RKPD Tahun ' + y + ' dengan data Rankhir Renja?\n\nPerubahan kustom pada Rankhir RKPD tahun ' + y + ' akan direset dan dicocokkan kembali ke data Rankhir Renja.')){
        return;
      }
      showLoading(true);
      const fd = new FormData();
      ...
      fetch(BASE_URL + 'Daerah/resetRankhirRKPDDataFromRenja', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
      })
  ...
  <?php if (!$IsLoggedIn || !isset($_SESSION['KodeWilayah'])) { ?>
    ...
    if (btnFilter) {
      btnFilter.addEventListener('click', function(){
        ...
        .then(function(res){
          if (res.trim() === '1') {
            let redirectUrl = BASE_URL + 'Daerah/RankhirRKPD';
            if (instansiVal) {
  ...
  <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
    ...
    if (btnFilterInstansi) {
      btnFilterInstansi.addEventListener('click', function(){
        ...
        let redirectUrl = BASE_URL + 'Daerah/RankhirRKPD?tahun=' + encodeURIComponent(tahun);
