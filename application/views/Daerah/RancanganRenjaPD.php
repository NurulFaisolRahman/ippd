...
<title>Rancangan Renja Perangkat Daerah</title>
  ...
  <div class="renja-wrapper">
    ...
    <div class="topbar">
      <div class="topbar-row">
        <div class="app-title">
          <h1>Rancangan Rencana Kerja (Renja) Perangkat Daerah</h1>
          <span>Data Tujuan, Sasaran, Urusan, Bidang, dan Program bersumber dari Renstra Perangkat Daerah &mdash; menampilkan Rancangan Renja Tahun <b id="activeYearLabel"><?= html_escape($TahunAktif) ?></b></span>
        </div>
        <div class="topbar-controls">
          ...
          <button type="button" id="btnSyncData" class="btn-sync" title="Cocokkan / Sinkronkan Ulang Data Rancangan Renja dengan Renja">
...
function renderColBadge(diffObj, fieldName){
  ...
  const title = 'Diubah di Rancangan RKPD' + (oldVal ? ('\nNilai semula di Renja: ' + oldVal) : '');
...
function saveKegiatan(){
  ...
  fetch(BASE_URL + 'Instansi/simpanRancanganRenjaKegiatan', {
    method: 'POST',
    body: fd,
...
function saveSubKegiatan(){
  ...
  fetch(BASE_URL + 'Instansi/simpanRancanganRenjaSubKegiatan', {
    method: 'POST',
    body: fd,
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
...
function loadRenjaDataServer(tahun, instansiId){
  showLoading(true);
  let url = BASE_URL + 'Instansi/getRancanganRenjaPDJson?tahun=' + encodeURIComponent(tahun);
  ...
  .then(function(json){
    ...
    if(json.status === 'success' && json.data){
      ...
      renderTable();
      showToast('Data Rancangan Renja TA ' + tahun + ' berhasil dimuat.');
    }
  })
  .catch(function(err){
    showLoading(false);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Sinkronisasi Ulang Data Rancangan Renja dengan Renja
  const btnSync = document.getElementById('btnSyncData');
  if(btnSync){
    btnSync.addEventListener('click', function(){
      const y = renjaData.tahunAktif || 2026;
      if(!confirm('Apakah Anda yakin ingin menyinkronkan ulang data Rancangan Renja Tahun ' + y + ' dengan data Ranwal Renja?\n\nData Rancangan Renja tahun ' + y + ' akan dicocokkan kembali mengambil data terbaru dari Ranwal Renja.')){
        return;
      }
      showLoading(true);
      const fd = new FormData();
      ...
      fetch(BASE_URL + 'Instansi/resetRancanganRenjaDataFromRenja', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
      })
  ...
  <?php if (!$IsLoggedIn || !isset($_SESSION['KodeWilayah'])) { ?>
    ...
    if (btnFilter) {
      btnFilter.addEventListener('click', function(){
        ...
        .then(function(res){
          if (res.trim() === '1') {
            let redirectUrl = BASE_URL + 'Instansi/RancanganRenjaPD';
            if (instansiVal) {
  ...
  <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
    ...
    if (btnFilterInstansi) {
      btnFilterInstansi.addEventListener('click', function(){
        ...
        let redirectUrl = BASE_URL + 'Instansi/RancanganRenjaPD?tahun=' + encodeURIComponent(tahun);
