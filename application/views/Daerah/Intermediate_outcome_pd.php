        ...
        <div class="container-fluid">
            ...
            <div style="background: white; padding: 12px 20px; margin-bottom: 25px; border: 1px solid #dee2e6; border-radius: 5px;">
                <div style="display: flex; gap: 25px; flex-wrap: wrap;">
                    ...
                    <a href="<?=base_url('Instansi/Intermediate_outcome_pd')?>" class="nav-link active">INTERMEDIATE OUTCOME PD (Level 2)</a>
                                        ...
                                        <?php $no = 1; foreach($items as $row): ?>
                                            <tr data-id="<?= (int)$row['id'] ?>">
                                                ...
                                                <td class="align-middle">
                                                    <?= nl2br(html_escape(substr($row['ultimate_kinerja'] ?? '—', 0, 100))) . (strlen($row['ultimate_kinerja'] ?? '') > 100 ? '...' : '') ?>
                                                </td>
                                                <td class="align-middle">
...
<!-- MODAL INTERMEDIATE OUTCOME (LEVEL 2) - HANYA UNTUK ROLE 4 -->
<?php if ($IsRole4) { ?>
<div class="modal fade" id="modalLevel2" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-xl" style="max-width: 1400px; width: 95%; margin: 30px auto;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="p-3 mb-4" style="background:#f3f3f3; border-radius:6px;">
                    ...
                    <div class="form-group">
                        ...
                        <select id="UltimateId" class="form-control select2-ultimate" style="width: 100%;">
                            <option value="">— Pilih Ultimate Outcome —</option>
                            <?php foreach ($ultimate_options as $opt): ?>
                                <option value="<?= $opt['id'] ?>"><?= html_escape(substr($opt['ultimate_kinerja'], 0, 150)) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="mb-2 text-muted small">
                            <em>Intermediate Outcome Level 2 - Kinerja Strategis Sektor</em>
                        </div>
                        <textarea id="kinerja" class="form-control" rows="4" placeholder="Masukkan kinerja strategis sektor..."></textarea>
                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="btn-simpan-level2">
                    Simpan Perubahan
                </button>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $("#Filter").click(function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res === '1') {
                    var url = BaseURL + "Instansi/Intermediate_outcome_pd";
    ...
    <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
        ...
        $("#ResetFilterBtn").click(function() { 
            window.location.href = BaseURL + "Instansi/Intermediate_outcome_pd"; 
        });
    <?php } ?>
    ...
    function loadDinasOptions(selectedDinasId = '') {
        $.ajax({
            url: BaseURL + 'Instansi/Intermediate_outcome_pd_get_daftar_dinas',
            type: 'GET',
            dataType: 'json',
            beforeSend: function() {
    ...
    function loadPelaksanaByDinas(dinasId, selectedPelaksanaId = '') {
        ...
        $.ajax({
            url: BaseURL + 'Instansi/Intermediate_outcome_pd_get_pelaksana_by_dinas',
            type: 'POST',
    ...
    function loadPerangkatDaerah(callback) {
        $.ajax({
            url: BaseURL + "Instansi/Intermediate_outcome_pd_get_perangkat_daerah",
            type: "GET",
            dataType: "json",
            success: function(res) {
    ...
    $(document).off('click', '.btn-edit').on('click', '.btn-edit', function(e) {
        ...
        $('#modalLevel2 .modal-content').append('<div class="modal-loading"><i class="fa fa-spinner fa-spin fa-3x text-primary"></i><br><span>Memuat data...</span></div>');
        
        $.ajax({
            url: BaseURL + 'Instansi/Intermediate_outcome_pd_get',
            type: 'POST',
            data: { id: id },
            dataType: 'json',
            success: function(res) {
                ...
                $('#id_level2').val(data.id);
                $('#UltimateId').val(data.ultimate_outcome_id || '').trigger('change');
                ...
                if (data.pelaksana) {
                    $.ajax({
                        url: BaseURL + 'Instansi/Intermediate_outcome_pd_get_pelaksana_detail',
                        type: 'POST',
    ...
    // ==================== SIMPAN DATA ====================
    $('#btn-simpan-level2').off('click').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        $.ajax({
            url: BaseURL + 'Instansi/Intermediate_outcome_pd_simpan',
            type: 'POST',
            data: {
                id: id,
    ...
    $(document).off('click', '.btn-hapus').on('click', '.btn-hapus', function(e) {
        ...
        $.ajax({
            url: BaseURL + 'Instansi/Intermediate_outcome_pd_hapus',
            type: 'POST',
            data: { id: id },
            dataType: 'json',
            success: function(res) {
    ...
    <?php } else { ?>
    // ================= MODE READONLY - HANYA FILTER =================
    // Inisialisasi DataTable untuk mode readonly
    if ($('#data-table-basic').length > 0) {
        try {
            if ($.fn.DataTable.isDataTable('#data-table-basic')) {
                $('#data-table-basic').DataTable().destroy();
