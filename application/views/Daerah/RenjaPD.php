  ...
  <div class="renja-wrapper">
    ...
    <div class="topbar">
      <div class="topbar-row">
        <div class="app-title">
          <h1>Rencana Kerja (Renja) Perangkat Daerah</h1>
          <span>Data Tujuan, Sasaran, Urusan, Bidang, dan Program bersumber dari Renstra Perangkat Daerah &mdash; menampilkan Ranwal Renja Tahun <b id="activeYearLabel"><?= html_escape($TahunAktif) ?></b></span>
        </div>
        <div class="topbar-controls">
          ...
          <button type="button" id="btnSyncData" class="btn-sync" title="Cocokkan / Sinkronkan Ulang Data Renja dengan Renstra">
            <i class="fa fa-refresh"></i> Sinkronkan Renstra
          </button>
        </div>
      </div>
    </div>
...
function renderColBadge(diffObj, fieldName){
  ...
  const title = 'Diubah di Ranwal RKPD' + (oldVal ? ('\nNilai semula di Renja: ' + oldVal) : '');
...
function saveKegiatan(){
  ...
  if (typeof CSRF_NAME !== 'undefined' && CSRF_NAME) {
  ...
  fetch(BASE_URL + 'Instansi/simpanRenjaKegiatan', {
    method: 'POST',
    body: fd,
...
function saveSubKegiatan(){
  ...
  // AJAX POST ke Server
  const dbSubId = sk.db_id || sk.id.replace('sub-', '');
  const dbKegId = k.db_id || k.id.replace('keg-', '');
  showLoading(true);
  ...
  fd.append('sub_kegiatan_id', dbSubId);
  fd.append('kegiatan_id', dbKegId);
  fd.append('tahun', renjaData.tahunAktif || 2026);
  fd.append('instansi_id', activeInstansiId);
  ...
  fetch(BASE_URL + 'Instansi/simpanRenjaSubKegiatan', {
    method: 'POST',
    body: fd,
    headers: { 'X-Requested-With': 'XMLHttpRequest' }
  })
  ...
  .then(function(data){
    ...
    if(data.status === 'success'){
      ...
      refreshSubKegiatanChain(currentSubId, currentSubParentId);
...
/* =========================================================================
   MEMUAT DATA RENJA DARI SERVER BERDASARKAN TAHUN & INSTANSI
   ========================================================================= */
function loadRenjaDataServer(tahun, instansiId){
  showLoading(true);
  const url = BASE_URL + 'Instansi/getRenjaPDJson?tahun=' + encodeURIComponent(tahun) + '&instansi_id=' + encodeURIComponent(instansiId);
  ...
  .then(function(json){
    ...
    if(json.status === 'success' && json.data){
      ...
      renderTable();
      showToast('Data Renja TA ' + tahun + ' berhasil dimuat.');
    }
  })
  .catch(function(err){
    showLoading(false);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Sinkronisasi Ulang Data Renja dengan Renstra
  const btnSync = document.getElementById('btnSyncData');
  if(btnSync){
    btnSync.addEventListener('click', function(){
      const y = renjaData.tahunAktif || 2026;
      if(!confirm('Apakah Anda yakin ingin menyinkronkan ulang data Ranwal Renja Tahun ' + y + ' dengan data Renstra?\n\nPerubahan kustom pada tahun ' + y + ' akan direset dan dicocokkan kembali ke data Renstra.')){
        return;
      }
      showLoading(true);
      const fd = new FormData();
      ...
      fetch(BASE_URL + 'Instansi/resetRenjaDataFromRenstra', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
      })
  ...
  <?php if (!$IsLoggedIn || !isset($_SESSION['KodeWilayah'])) { ?>
    ...
    if (btnFilter) {
      btnFilter.addEventListener('click', function(){
        ...
        .then(function(res){
          if (res.trim() === '1') {
            let redirectUrl = BASE_URL + 'Instansi/RenjaPD';
            if (instansiVal) {
  ...
  <?php if ($IsLoggedIn && !$IsRole4 && !empty($KodeWilayah) && !empty($ListInstansi)) { ?>
    ...
    if (btnFilterInstansi) {
      btnFilterInstansi.addEventListener('click', function(){
        ...
        let redirectUrl = BASE_URL + 'Instansi/RenjaPD?tahun=' + encodeURIComponent(tahun);
